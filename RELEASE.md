...
- **Revolving light-trap fabrication blueprint set + metal-cap / lap-joint redesign + integrated frame
  ...
  **Rivet-nut symbol fix:** the `draw_bolt(end="rivnut")` symbol was redrawn to read as a real rivet-nut — an internally-threaded **barrel aligned with (spanning) the wall thickness** + a low **pancake head** (same profile as the blind-rivet factory head) seated on the **inner (bore-side) edge** — the bolt threads up into it; drawn in a **distinct bronze** so it reads apart from the silver blind rivets. A `wall=` arg passes the drawn metal thickness so the barrel aligns with it (Sheets 5 & 10 + the Sheet 5 rivet-nut inset). Weight/CG + the §6b swing-cage frame-steel itemization still tracked in `TODO.md`. *(In progress.)*
