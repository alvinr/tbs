...
- **Sheet cleanups.** Sheet 1's Section A-A duplicated Sheet 3's wall section (a superset), so Sheet 1 is now a single front-view general arrangement pointing to Sheet 3 for the section. On Sheet 4 the shutter mount is drawn as a proper lens cross-section: front element → a converging front-cell funnel through the board hole, front flange + rear retaining ring with clamp-force arrows, and the rear element protruding past the opening; the glass groups are labeled "front/rear element" with a one-line definition.
