...
- **Front board: tilt-swing eliminated → a quick-change pinhole disc holder that also takes large-format lenses.** A pinhole is a point aperture — the image is a central projection *through the pinhole point*, independent of the plate's orientation — so tilting a pinhole board is optically inert (~2.3mm parallax); all perspective control lives in the film plane. The front board is now a compact **pinhole disc holder**: a **Ø180 × 18mm** 6061 plate (Ø110 scene-side taper → **Ø72** light aperture) with a **Ø82 × 6mm** counterbore that seats an interchangeable **Ø80 × 5mm carrier** — either a pinhole board (SS-302 shim over a central bore, Ø2.17/1.5/3.0) **or** a **Copal/Compur 0/1/3 lens board** (34.6 / 41.6 / **65.0**mm holes) — clamped against a neoprene light-seal washer (Ø82 × Ø72 × 1.5) by a **Ø120** retaining ring on **4× M6 knurled thumb screws @ Ø100** (30-second, tool-free swap). It bolts (4× M6 @ Ø150) to a **Ø240 × 6mm** steel wall-frame adapter welded over the corrugation. This one holder **retires both** the tilt-swing front board (`TSB_*`/`FRONT_BOARD_*` constants, the wrong-physics distortion generators + `tilt-swing-board-analysis.md`) **and** the standard **Ø600** interchangeable pinhole/lens plate (`generate_plate_drawing.py`, `PLATE_OD`/`WALL_FRAME_T`/`PINHOLE_DISC_*`). Costs reconciled across `parts.py`/`costing.py`: front board **$305–$636** (down from the tilt-swing $1,604–$2,459), the `optics` system trimmed to the wall interface (adapter + dark slide), a pinhole double-count fixed — grand total **−$80/$118/$160**. Reports, funding proposal and project summary rewritten to the correct optics; all files renamed `tilt-swing*` → `pinhole-disc-holder*`.
- **Disc holder — full buildable drawing set + interactive 3D model.** A 4-sheet TBS-PDH set: **Sheet 1** front-view general arrangement, **Sheet 2** fabrication blueprints (plate / retaining ring / carrier, fully dimensioned), **Sheet 3** the wall-frame adapter + a section through the container wall (corrugation → welded adapter → bolted plate → carrier/ring/seal, with the light path, thumb screws and mount bolts), **Sheet 4** the Copal/Compur lens board + a lens cross-section (a straight lens barrel with an integral flange through the board hole, a rear retaining ring clamping the board, elements at the barrel ends). CNC-shop tolerance block on the fab sheets. Engineering resolved along the way: the **Ø72 aperture clears a Copal #3 barrel** (hole spec corrected to the standard **65.0mm** per [lensnotes.com](https://lensnotes.com/photography/lens-board/)/Compur, over Intrepid's 61.5); the thumb-screw **taps at Ø100** give ≥1.5×D edge distance to the Ø82 counterbore; the **Ø80×5 carrier fills the seat** (`PDH_DISC_T`, derived + ~0.5mm proud) so the ring actually compresses the light seal — all confirmed against real Compur-shutter lens-board photos. Interactive Sketchfab model (uid `4ce663d3…`): click the ring to pull it + the thumb screws out and reveal the carrier and the plate's four tapped holes. All four affected models — **overview, pinhole-disc-holder, construction, water** — re-sent and re-uploaded to Sketchfab (same URLs).
