...
- **Revolving light-trap fabrication blueprint set + metal-cap / lap-joint redesign + integrated frame
  ...
  **Bearing outer-race retention (located/floating):** the drawings only retained the *inner* race (shaft circlips) — the outer race was a plain Ø130 H7 bore with **no axial retention**, which wouldn't hold the drum's hanging weight. Added the standard located/floating scheme: the **UPPER bearing is LOCATED** — the outer race seats on a machined **Ø122 shoulder** (drum side, carries the hang) + a **DIN 472 retaining ring** (beam side); the **LOWER bearing FLOATS** (plain Ø130 H7 bore). Shown on the Sheet 5 hub + fully dimensioned on the Sheet 6 ring blueprint; the upper ring grew 30→38mm for the shoulder + gap. New `ll-bearing-rings` line — the retention hardware was previously uncosted — **firmed to McMaster 90154A895** (external, Ø75 shaft, $7.05 — inner) **+ 98455A170** (internal, Ø130 bore, $7.26 — outer), ≈$14; datasheets verified (the two SKUs were transposed in the source note) and the **bore groove corrected Ø139→Ø134 × 4.15** with the shaft groove Ø72.0 × 2.65 added to the Sheet 6 blueprint. lightlock → $2,959–$3,983. Follow-up review fixes: the Sheet 5 **circlips redrawn as external clips standing proud of the shaft** (were drawn projecting into it); the Sheet 6 lower-collar bolts moved **onto the bolt PCD** (were too near the OD edge); the shoulder/groove **dimensions moved into the machining notes** (the tiny section callouts read poorly); and the lower-hub label clarified that the **outer race deliberately floats** (radially located, captive on the shaft via the circlips — the upper bearing is the located one). *(In progress.)*
  **More review fixes:** the **cap→flange joint is now a through-bolt** — countersunk in the cap, Ø11 clearance through the steel flange, nut behind (drilled through, simpler than a blind-tapped flange; Sheets 5/6/10 + fastener line + report). Sheet 10's **axle beam is shown open-ended at the drum axis and the corner post open at the bottom** (`hollow_beam_long` gained an `open_ends` option, so the cut tube ends read open, not capped). The Sheet 9 **TEK screw redrawn to the ibc-frame convention** (hex-washer head + shank + thread ticks). The Sheet 4 brush-holder now names its part number (**Tanis/Gordon AH400436**). *(In progress.)*
  **Stile→cap joint made buildable (Sheet 11 View A):** the old scheme buried an M12 inside the tube (no wrench access). Reworked to a **tapped plug** secured by a single **M10 countersunk bolt driven from the cap's outside face** (wrench-accessible), through the cap into the plug — sealed with DP8010 for light-tightness — with **2× grub (set) screws** through the tube wall locking the plug + preventing rotation. Sheet 11 + Sheet 1 main view + report + `parts.py` reconciled (F4 M12→M10-CSK-from-F1 + M8 grub screws; fastener lot $91→$87, grub SKU pending). *(In progress.)*
  **New Sheet 12 — Seal details (enlarged):** the two cramped seal cross-sections under the Sheet 7 light-path plans (the running-gap brush radial section + the top-end neoprene axial section) were pulled onto their **own Sheet 12**, drawn **1.5:1** in blueprint style with a title block so the geometry reads clearly; Sheet 7 keeps a boxed "SEE SHEET 12" pointer in the freed space. The light-trap set is now **12 sheets** (all title blocks bumped to "SHEET x OF 12"). Registered in the gallery / publish / setup / `dependencies.yml` and embedded in the report §9. *(In progress.)*
  **Bearing hang: bolted end-retainer plate (Sheet 5/6):** answering the "the whole weight is held by one circlip" concern, the UPPER (located) hub's beam-side circlip is **replaced by a bolted END-RETAINER PLATE** — a Ø90×4 steel disc bolted to the shaft end by a central M10 CSK screw (into a new M10×16 tapped hole in the beam-side shaft end), its rim clamping the SKF 6215 inner-race face so the drum's hang runs through a positive bolted member, not a lone circlip (transport-shock robust). The upper shaft is faced flush to the race + keeps only its drum-side circlip; the lower (floating) hub is unchanged (two circlips). To give the plate room, the **UPPER bearing ring's beam-side standoff was raised 38→45mm** (lower collar stays 38mm), lifting the top cross-beam ~7mm so the rotating retainer clears the fixed beam by **≈10mm** (welding-tolerance margin, up from ~3.5mm) — drum/bearing/`DRUM_H` unchanged, so no 3D re-send (the 3D cage models the bearing mount only generically). Sheet 5 hub + Sheet 6 stub-shaft + upper-ring blueprints (tapped-hole callout, LOWER/UPPER groove note, 45mm ring) + report retention table/prose reconciled; `parts.py` `ll-bearing-rings` → **$14→$17/$18** (−1 external ring, +$3 plate blank; M10 CSK from the F1 pack), lightlock **$2,955–$3,979 → $2,958–$3,983**, all gates green. *(In progress.)*
  **Review fixes (fasteners + seal-detail clarity):** (1) **cap→flange reverts to a TAPPED flange** — the flange is a machined steel part, so the CSK cap bolt threads straight into it (no nut, bolt flush), simpler than the through-drill + nut; Sheets 5/6/10 + `parts.py` F1 + report reconciled (no cost change — the shared nut came from the F7 pack). (2) **Sheet 12 detail-1 rivets** redrawn with the `blind_rivet` convention (factory head on the flange, set head in the drum) instead of plain lines. (3) **Sheet 12 detail-2** now shows how the neoprene wiper is secured — a PSA adhesive bond line (McMaster 93855K6, adhesive-backed) to the rotating drum cap + silicone bead, free edge sweeping the fixed frame plate (leader + note). (4) **Sheet 11 View A** — the plug leader was pointing at a grub screw; split into a plug leader (into the plug) + a labeled **2× M8 grub-screw** callout. (6) **Sheet 6 ring/collar bolts** drawn **countersunk** (flush in the ring face) to match the rest of the set. *(In progress.)*
  **Rim-angle → beam: TEK screw → SS blind rivet:** the rim-angle→beam TEK self-driller is replaced by an **Ø1/8" 18-8 SS blind rivet (McMaster 97525A425**, datasheet on file). The beam is a closed 3mm RHS, so a blind rivet is the natural fit — set from below, the set head forms inside the bore (no internal access, unlike a weld-nut) and bears on the full wall (vs a self-driller's ~2 threads in 3mm); SS bites the steel far better than an aluminum rivet, and it makes the whole rim joint one fastener language (the housing→rim laps are already SS blind rivets). Grip 4.8–6.35mm fits the 3mm-Al-leg + 3mm-steel stack. Material audit alongside: the structural lap rivets **are 18-8 SS** (97525A425 shell→cap, 97525A435 housing→frame — datasheets confirm), while the **brush-holder tack rivets are aluminum** (97447A015, *Aluminum Blind Rivets with Aluminum Mandrel* — datasheet confirms; aluminum is correct there, tacking an Al flange to the HDPE drum with no steel in the joint). Sheet 9 Section A-A (blind-rivet symbol + label + note) + Sheet 10 note + report reconciled; `ll-rim-teks`→`ll-rim-rivets` **$8–12 est → $14 firm** (97525A425 100-pack), lightlock **$2,958–$3,983 → $2,964–$3,985**, all gates green. *(In progress.)*
  **Review fixes (fastener drawing + hollow-wall inserts + circlip clarity):** (2/6) **countersunk bolts now draw as ONE continuous shape in one color** — `draw_bolt`'s CSK head was a separate patch in a different fill; merged head+shank into a single polygon (fixes every CSK bolt across the set, incl. the Sheet 6 collar bolts). (3) **Sheet 10 cap→flange bolt lengthened through the full 15mm flange** (through-tapped — simpler than a blind partial hole). (1) **handle feet → rivet-nuts:** the pull handle bolts into the hollow 5mm stile wall, so the feet now take **1/4″-20 rivet-nuts** (full thread engagement, can't strip the thin wall) instead of a direct tap — Sheet 11 View B + parts F5/`ll-grab-rail`/`ll-handle-stile` + report (fasteners lot **$87→$95**). (4) **Sheet 6 upper-ring DIN 472 circlip re-oriented** to seat in a groove and project INWARD into the bore (was projecting into the wall). (5) **new CIRCLIP DETAIL close-up (5:1) on Sheet 6** — the DIN 471 external ring seated in the Ø72.0×2.65 shaft groove, projecting past the Ø75 OD to retain the inner race, dimensioned blueprint-style. (7) **Sheet 4 brush→HDPE-shell securing shown** — the Al holder flange is **DP8010-BONDED (primary, spreads the load so nothing pulls through the soft 3mm HDPE) + blind-riveted with a backup washer** on the inside face; drawn in Section A-A + the HOLDER PROFILE inset, `ll-wiper-holder`/`ll-holder-rivets` + report reconciled (**+$5** backup washers). lightlock **$2,964–$3,985 → $2,977–$3,998**, all gates green. *(In progress.)*
  **Label tooling — render-based `--overflow` pass + light-trap label tidy:** `tidy_labels.py` gained a **`--overflow`** mode — the static `--check`/`--fix` half can't see *where* a label lands, so this one RENDERS every sheet a generator draws (monkeypatched `Figure.savefig` via `runpy`; no files written) and flags any label off the frame edge (OVERFLOW) or overlapping another (CROWDED), skipping tiny inset axes. Wired into `skill_tidy_labels.md` (pipeline step 1b) + `skill_label_placement.md` (self-review; plus a codified P8 refinement — notes-box shape follows the margin aspect, and a 2026-08 re-audit note). A full sweep of all 41 generators surfaced ~49 genuinely off-frame labels (backlogged in `TODO.md`, deferred past the light-trap). Applied it to the light-trap: **Sheet 6 notes** pulled back inside the frame (`width 2000→1560`, they were overflowing ~18%), Sheet 11 handle / Sheet 12 neoprene / Sheet 5 rivet-nut / Sheet 4 brush leaders shortened + reined in — off-frame labels 16→7 (worst +19%→+7%, all now tight-bbox margin). *(In progress.)*
  **Light-trap 3D model confirmed in sync + `LT_CAP_T` retired.** Audited the light-trap 3D model against the
  finished 2D blueprint: it already models the split metal caps (both 8mm Al), rim-angle lip, edge channels,
  cage frame, brush seals, and stile/handle — `manifest.py --check` (all 10 `.skp` in sync) + `check_consistency.py`
  (no 2D↔3D divergence) confirm it needs **no re-send** (the metal-cap cascade + send were done in the earlier
