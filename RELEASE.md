...
- **Pinhole disc holder now takes large-format lenses; standard Ø600 plate retired (A2).** The interchangeable carrier grows Ø50→**Ø80** and the aperture Ø40→**Ø65** so the same compact holder takes a pinhole board OR a **Copal/Compur 0/1/3 lens board** (34.6/41.6/61.5mm holes; ref: Intrepid Camera Lens Explorer), with retention sized to carry a lens's weight (6mm seat, Ø100×6 ring, M6 thumb screws). This lets it **supersede the standard Ø600 interchangeable pinhole/lens plate** — retired `generate_plate_drawing.py` + its sheets, the `PLATE_OD/WALL_FRAME_T/PINHOLE_DISC_*` constants, and rewrote `pinhole-report.md` to the wall + front-board mount interface (focus for a lens moves to the film-plane depth rail). Front-board $305–$636. Reconciled the `parts.py` **"optics"** system to the pinhole-WALL interface only (wall-frame adapter + dark slide, $35–$70; retired the Ø600 plate's 8× M12 / backing plate / M52 ring, and fixed the pinhole double-count by moving the element to front-board) — grand total −$80/$118/$160. Overview + water .skp re-uploaded to Sketchfab; *construction + disc-holder .skp re-uploads still pending.*
- **Pinhole disc holder — interactive 3D model + 4-bolt plate.** Built and uploaded the `pinhole-disc-holder` SketchUp model (Sketchfab uid `4ce663d3…`): a click Dynamic Component pulls the retaining ring + thumb screws out to reveal the interchangeable disc and the plate's four tapped holes; the ring bore tapers open toward the interior (the diverging light cone's exit), with clearances that remove orbit z-fighting. Simplified the plate mount to a **4-bolt pattern** (dropped the 2 registration dowels) — cascaded through the constants, 2D sheets, `parts.py`, report, dependency map and costing; front-board total now **$335–$726**.
- **Disc-holder blueprint completed to buildable.** Added the container-wall interface and shop data the set was missing: the **wall-frame adapter** (Ø240×6 backing ring, 4× M6 into the corrugation, Ø120 aperture) drawn into `overview.skp` with its through-bolts; a new **Sheet 3** section through the container wall (corrugation → welded adapter → bolted plate → carrier/ring, with the light path); a new **Sheet 4** Copal/Compur lens board (real drilled bore + front-flange / rear-retaining-ring mount mechanism, ref: S.K. Grimes); and **CNC-shop tolerances** on the fabrication sheets. Moved the taps to Ø94 PCD (ring Ø110) so the Ø82 counterbore no longer crosses them. *overview/water/construction .skp re-send + Sketchfab re-upload still pending.*
- **Copal #3 spec corrected + aperture opened to clear it.** Cross-checked the lens-board holes against [lensnotes.com](https://lensnotes.com/photography/lens-board/): Copal #3 is the standard **65.0mm** (matching Compur #3), not the 61.5mm taken from Intrepid. Since the front-plate aperture was Ø65 — exactly the #3 shutter barrel — grew it to **Ø72** (and coupled the neoprene washer bore to it, `PDH_WASHER_ID = PDH_APERTURE`, so it can't drift) so a Copal #3 barrel clears with margin. Cascaded through the constants, all 4 sheets (the tolerance block's bore/PCD literals are now constant f-exprs), `parts.py` (plate/washer/lens-board/CNC specs) and the report (fit table, tolerances, references). Confirmed against real Compur-shutter lens-board photos (front flange on the subject face, retaining ring threaded on from the back). Legibility passes: enlarged Sheet 1's Section A-A and drew its thumb screws to scale (Ø6 shaft into a ~10mm plate tap + Ø12 knurled head, now labeled), and enlarged Sheet 4's shutter-mount section ~50%. *disc-holder .skp re-send pending.*
- **Thumb-screw edge distance fixed; ring widened.** The M6 taps at Ø94 sat only 6mm from the Ø82 counterbore wall — under the ~1.5×D minimum edge distance for an M6 tap. Moved the taps out to **Ø100** (9mm clearance) and widened the retaining ring OD **Ø110→Ø120** to house them with a ~10mm outer margin. Cascaded to the constants, all 4 sheets, `parts.py` (ring/CNC specs), master BOM and the report. Also drew the thumb screws into Sheet 3's wall section (they were missing — the ring's attachment to the plate now reads) and added a note on Sheet 4 explaining the front/rear lens *cells*. *disc-holder + overview .skp re-send pending.*
- **Sheet cleanups.** Sheet 1's Section A-A duplicated Sheet 3's wall section (a superset), so Sheet 1 is now a single front-view general arrangement pointing to Sheet 3 for the section. Sheet 3's title block sits at the very bottom of the sheet with the CNC tolerance block above it (was inverted). On Sheet 4 the shutter mount is drawn as a proper lens cross-section: a straight lens barrel through the board hole with the front element at its left (subject) end and the rear element protruding past the opening on the film side, plus a barrel flange + rear retaining ring with clamp-force arrows; the glass groups are labeled "front/rear element".
- **Carrier standardized so the light seal actually clamps.** The washer seals the carrier's perimeter against the seat floor, but a thin carrier left the 6mm seat under-filled — a flat ring resting on the plate face couldn't reach it, so the washer never compressed. Every carrier is now a **Ø80 × 5mm** disc (`PDH_DISC_T`, derived to fill the seat + ~0.5mm proud): a pinhole carrier is a 5mm disc with the SS-302 shim over a central bore, the lens board is a 5mm disc drilled for the shutter, so the ring compresses the seal and seats every carrier identically. Cascaded to the constant, 2D sheets (Sheet 2 carrier, Sheet 3 section), the disc-holder model, `parts.py` and the report (§2/§3/§5). Retaining fasteners stay tool-free knurled thumb screws (confirmed).
...
- **Fixed housing redesigned to a weld-free "slide-together" kit.** The extrusion-welded HDPE tube is
  ...
  H-mullions, with the sill/header band pieces glued +
  ...
  strips run rail-to-rail across each opening so the mullions and caps bolt to
  a member.
- **Floor/roof caps simplified to plain plates; HDPE surround attachment reconciled.** The caps are plain HDPE
  ...
  blind-riveting the **drum cage** (which wraps it and is welded into the panel frame).
- **Drum & housing opening edges framed; housing channels moved bonded → rivet-only.** The rotating drum's 80°
  opening jambs gain a riveted 6063-T5 aluminum U-channel (new `ll-drum-channel`).
...
- **Prep shelf redesigned as a plywood-primary structure.** Removed the welded 25×25×3 steel perimeter frame,
  ...
  cam-latch; validated by `chem_shelf_load.py` (board SF 44, chain 5.4, tee-nut 20). `SHELF_T` 22→18.
