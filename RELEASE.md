...
- **Revolving light-trap fabrication blueprint set + metal-cap / lap-joint redesign + integrated frame
  ...
  **Rivet-nut symbol fix:** the `draw_bolt(end="rivnut")` symbol was redrawn to match the real McMaster rivet-nut profile — a **flange seated on the near (accessible) wall face** + an internally-threaded **barrel that passes through the wall and protrudes into the bore** (the bolt threads into the barrel), in a **distinct bronze** so it reads apart from the silver blind rivets; the ring/collar→beam positions moved the flange onto the near face so the fastener visibly engages through to the bore side (Sheets 5 & 10 + the Sheet 5 rivet-nut inset). Weight/CG + the §6b swing-cage frame-steel itemization still tracked in `TODO.md`. *(In progress.)*
