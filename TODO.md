...
- [ ] **Film-plane corner — recompute + review the load case (blueprint Phase 1c, still OPEN).** Two engineering items the corner blueprint left to settle need calculation and a fresh review: (1) the **per-corner load** from the reconciled moving mass (film carriage ~57 kg incl. ACM, from the weight model) and the **cross-slide bending safety factor** for the ¼"×1½" 304 flat bar — the number the milestone was meant to "firm"; (2) **verify the `XSLIDE_*` travel values** (Z/X stroke) against the actual ±40° tilt / ±28° swing geometry before promoting them `reserved → firm`. See `fp-corner-blueprint-spec.md` §Phase 1c + §Open items. (Flagged 2026-08-12.)
