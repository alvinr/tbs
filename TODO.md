...
- [ ] **Push electrical.skp to Sketchfab (2026-09-09).** electrical was re-sent + saved + committed (`d74606b2`) with the updated chem-shelf context; `manifest.py --check` is clean. ONLY the Sketchfab render is stale — the push was refused because the live doc had switched to lighttrap. Next time electrical is the active doc: `push_sketchfab.py electrical` (no commit needed — uid/manifest unchanged). Not urgent — context use of the shelf.
