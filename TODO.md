...
- [x] **Re-audit the redesigned IBC-restraint solids — DONE (2026-08-18, live ibc-stack).** The cleat/hanger
  redesign (L-cleats both ends, 16 backing plates, inside-aligned wall hangers, cleat-clear D-rings) added
  ~76 solids (155→231). `check_interference.py --write` confirms **none of the 10 interferences involve any
...
- [x] **Front-bar J2/J7 bolt M12×40 → M12×65 18-8 SS — SKU FIRM 2026-08-17 (Alvin).** The J2 cleat + J7
  wall-end bolts span the ~58mm bar section, so M12×40 was too short → **M12×65**. Firmed to **McMaster
  92800A481** (M12×1.75×65 18-8 SS, partial thread, $9.95/pack of 5 = $1.99/ea; full-thread alt 92314A595
  $11.92/5). `parts.py` `bolt-m12x65-ss` + costing + report J2/J7 + worklist stamped. (The hollow-bar crush
  joint review stays a Bucket-2 quote-phase item.)
...
- [x] **Phase D — fab-detail sheets DONE.** (D-1) Member CUT LIST — report §3.7, generated by
  `ibc_frame_cutlist.py` from the constants (uprights/rings/feet/bars/hangers/backing plates/cleats + a stock
  linear-feet summary). (D-2) per-part fab detail sheets — satisfied by the existing 1:1 **Plate Fabrication
  Schedule** (foot/backing/hanger = Plates 1–3, cleat = Plate 4); reconciled the stale PLATE 4 (was a 50×50×6
  rolled angle) to the current fabricated 8mm-plate L (90 leg + 59 upstand, 20 wide, M12×65 @40) matching the
  3D `tote_restraint`; dropped the redundant draft "Sheet 7" (no duplication). (D-3) consolidated WELD MAP —
  new **Sheet 6** (W1–W6 ticked to their frame locations + the full schedule). (D-4) dimensional VERIFY —
  cut-list + plate-schedule dims cross-check clean against the shared constants (`IBC_FRAME_RHS`,
  `IBC_FRONT_BAR_D`, `IBC_FOOT_*`) that `ibc-stack.skp` builds from; `check_consistency.py` shows no new stale
  literal. Residual: a live-model orbit eyeball of `ibc-stack.skp` (Alvin, when that doc is open) — geometry is
  constant-derived so nothing to fix, just a visual confirm. Then Phase E (optional polish). Smaller items:
  **hollow-bar bolt-crush** joint review (Bucket 2 / quote), the **surface-finish/coating** spec (set at
  fab/quote). (M12×65 SS SKU — DONE, 92800A481.)
