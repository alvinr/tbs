...
- [ ] **Pump-run support-detail fabrication sheet (#29 follow-up).** The support boards currently show **edge-on** in the panel-layout front elevation + are described in plumbing-report §5.3, but there is no dedicated fabrication drawing. Add a 2D **support detail** — a side elevation of each board (dimensions, Z extent), the **L-bracket flush mount** section (post inner face → welded leg → landing leg → ply, seated flush), and the **P-clip spacing** — so a fabricator can build it directly. Basis already prototyped in the scratchpad (`p02_board.png` / `p02_board_near.png`). New registered diagram: generator (`generate_panel_layout.py` or its own) → gallery (`all-diagrams.md`) + `mkdocs.yml` + `setup_docs.py` + `publish.sh` + `dependencies.yml`. (Alvin 2026-08-09.)
