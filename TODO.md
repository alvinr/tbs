...
- [ ] **Phase D — fab-detail sheets (in progress).** (D-1) Member CUT LIST **DONE** — report §3.7, generated by
  `ibc_frame_cutlist.py` from the constants (uprights/rings/feet/bars/hangers/backing plates/cleats + a stock
  linear-feet summary). REMAINING: (D-2) per-part fab detail sheets (foot plate, wall-hanger fold, exterior
  backing plate, front-bar **cleat**); (D-3) consolidated WELD MAP; (D-4) dimensional VERIFY vs `ibc-stack.skp`.
  Then Phase E (optional polish). Smaller items: **hollow-bar bolt-crush** joint review (Bucket 2 / quote), the
  **surface-finish/coating** spec (set at fab/quote). (M12×65 SS SKU — DONE, 92800A481.)
