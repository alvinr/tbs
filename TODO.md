...
- [ ] **Draw the processing-tray center-seam joint detail (design DONE 2026-07-21; drawing pending).** Joint resolved: a **~40mm shingle-oriented lap** (the uphill panel laps *over* the downhill one so water sheets off the step on the sloped floor), **silicone-bedded**, bolted with **12× M6×16 + serrated flange nuts** underneath — `bolt-m6-tray` pinned to **M6×16**; report §2.1/§2.6 + parts updated. **Remaining:** add a new detail sheet (lap-joint cross-section: 2 overlapped 1.5mm panels + silicone bed + M6 + flange nut, shingle orientation vs the slope) and register it (gallery + `publish.sh` + `setup_docs.py` + `all-diagrams.md`). (Alvin 2026-07-21.)
