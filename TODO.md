...
- [ ] **Deferred Sketchfab pushes — electrical + construction.** Both are re-sent + saved + committed with current geometry (`manifest.py --check` clean); ONLY the Sketchfab render lags (each push was skipped because a *different* doc was the live one at commit time). Next time each is the active doc: `push_sketchfab.py electrical` / `push_sketchfab.py construction` (no commit needed — uid/manifest unchanged). electrical = chem-shelf context (`d74606b2`); construction = phase-5 light-trap split (`67b79f9c`). Not urgent.
