...
- [ ] **Re-send construction + electrical .skp for the chem-shelf chain-stay change (2026-09-07).** `shelf()` was rebuilt (ply-primary chain stays) and overview was re-sent + saved + committed (`87f65c31`). `construction.skp` and `electrical.skp` both embed `ov.shelf()` and are flagged STALE by `manifest.py --check`. Re-send each (open the doc → `--send` → save + Sketchfab reload → `manifest.py --update construction electrical` → commit the two `.skp`). Not urgent — they're context/phase uses of the shelf, not the focus model.
