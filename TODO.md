...
- [ ] **Film-plane corner — load case COMPUTED, pending Alvin's review (blueprint Phase 1c).** Calculated in `src/generators/fp_corner_load.py` (renders `diagrams/fp-corner-load-case.png`), driven from constants: **C2 travel verified** — Z (tilt ±40°) = 245mm and X (swing ±28°) = 257mm exactly match `XSLIDE_Z/X_TRAVEL` (promote `reserved→firm` on sign-off). **C1** — per-corner load W/4 = 124N → X-slide bending: **DEEP mount (38.1mm ⟂ load) SF≈10 / δ≈0.1mm; FLAT SF≈1.7 (marginal, fails ×2)**. ACTION FOR ALVIN: review the load case + **confirm the bars are locked DEEP** (strong axis); then record `reserved→firm` in the blueprint spec §Phase 1c and add a "bars mounted deep" callout to Sheet 8 + the parts spec. (Computed 2026-08-12.)
