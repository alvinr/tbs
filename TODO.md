...
- [ ] **Save + re-send lighttrap/construction/overview .skp for the housing-skin extension + support strips (2026-09-08).** `drum_housing()` now extends the skin to the beam outer faces (notched at 90/270) + adds 4 channel-support strips + drops the housing rim-angle; the model was `--send` (live doc current) but not saved. `manifest.py --check` flags **lighttrap** (focus) + **construction** + **overview** (they import the drum) STALE. Focus-first: open lighttrap → confirm title → (no re-send needed, already sent; `--send` only if the live doc was clobbered) → save → `push_sketchfab.py lighttrap` → `manifest.py --update lighttrap` → commit `.skp` + `dependencies.yml`. Then re-send overview + construction (they need a fresh `--send` for the drum change) → save → push → `manifest.py --update` → commit.
- [ ] **Re-send electrical.skp for the chem-shelf chain-stay change (2026-09-07).** `shelf()` was rebuilt (ply-primary chain stays); overview (`87f65c31`) and construction (`7addad7e`) are re-sent + saved + pushed + committed. `electrical.skp` embeds `ov.shelf()` as context and is still flagged STALE by `manifest.py --check`. Open electrical → `--send` (may need a Bash-permission OK) → save → `push_sketchfab.py electrical` → `manifest.py --update electrical` → commit the `.skp`. Not urgent — context use of the shelf, not the focus model.
