...
- [ ] **Flexible connectors on BOTH sides of every pump.** The pumps (P-01…P-05 + the recycle/spray pumps) vibrate in service; rigidly-coupled solvent-weld PVC joints will **fatigue-crack at the glued joints**. Add a **flexible connector on each side (suction + discharge) of every pump** — the same corrugated flexible-jumper detail already used on the **IBC totes** (see the completed "IBC-tote FLEXIBLE-JUMPER" work in Bucket 1). Isolates pump vibration from the rigid run. Cascade: 3D models (a flex jumper at each pump port on water/overview/corridor/construction), the P&ID schematic, `parts.py` (jumper qty × 2 per pump), master shopping list + cost. Confirm the jumper's pressure/temperature rating suits the pump discharge side.
- [ ] **Captive T-nuts for all plywood-mounted components.** Every component that machine-screws to plywood — filters (Pentair 150061 brackets), the P-04/SV-02/DV-02 skid row, ACC-01/ACC-02, the corridor pumps' mount shirt, valve brackets, the EP panel, the shelf, the Fan-B band — should land on a **captive T-nut** (pronged/blind or threaded insert) set into the ply, not a wood/lag screw, wherever a removable/re-torqueable machine-screw joint is wanted (serviceability, no thread-strip in ply). **Action:** enumerate every ply-mount interface, pick the **thread size + pitch per component** (match each component's own fastener — e.g. M6/M8 for the filter brackets, ¼-20 for pump-rail/shelf, etc.), confirm 18mm ply vs T-nut barrel length + front-vs-back install, then **add + price** the T-nuts (qty per interface) in `parts.py` → cascade BOM / master-shopping-list / cost. (Alvin 2026-08-05.)
