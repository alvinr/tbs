...
- [ ] **Add the processing-tray center-flange bolted joint to a 2D diagram.** The tray is two SS panels butted + **field-bolted at a center flange** (12× M6 flange bolts `bolt-m6-tray` + silicone gasket — `processing-tray-and-spray-bar.md` §on the two-panel split), but **no sheet depicts it** — the tray is drawn only as a welded pan (`spray-bar-sheet2` section, `-sheet3` plan). Add a center-flange joint detail (the two panel edges + flange + M6 bolt pattern + silicone gasket) so the field assembly is a buildable drawing, then register the new sheet (gallery + `publish.sh` + `setup_docs.py` + `all-diagrams.md`). (Alvin 2026-07-21.)
- [ ] **Design the spray-arm pinch-clamp geometry (blocks the M6 pinch-bolt length).** `bolt-m6-pinch` clamps the Ø25×2 arm tube onto its ball-joint stud, but the clamp itself — split/slit vs two-ear boss, ear projection, slit width — was never designed or drawn (`spray-bar-sheet2` shows only a decorative bolt stub). Design the pinch detail; the bolt length follows. (Alvin 2026-07-21.)
- [ ] **Resolve the pinhole END-wall mount detail for the flush power panel + shelf cleat (blocks 2 bolt lengths).** `bolt-m6-panel` (flush power panel) and `bolt-m8-wall` (shelf hinge cleat) both fasten to the pinhole END wall (36mm trapezium corrugation). Length swings ~3× on one question: does the bolt land on a **flat backing/rib** (grip ≈ 8mm → M6×20 / M8×20–25) or **bridge the corrugation** (grip up to ~42mm → M6×55 / M8×55)? Decide the mount (weld a flat backing plate/frame behind each? bolt to a rib flange?), draw it, then pin both lengths. Same open question as the M12 side-wall through-bolts, but on the END wall. (Alvin 2026-07-21.)
