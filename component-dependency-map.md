...
| **film-plane** | `src/models/generate_film_plane_model.py` | `models/film-plane.skp` + `src/models/film-plane.rb` | Film plane + **tilt/swing corner mechanism** — the WHOLE plane at container scale (reconciled to the built **304 U-channel / acetal-skate / 316-cross-slide / Ruland-U-joint** design 2026-07; **was** the Option-A leadscrew DC). The bolt-level single-corner detail lives in the **film-plane-mechanism** model (next row); this model carries the same corner design across the whole plane. The film is a **FIXED-SIZE rigid rectangle** (FP_W wide × FP_H rail-to-rail) that only changes ANGLE; each corner rides an **acetal 4-wheel skate inside a 304 U-channel depth rail** (depth/focus), coupled through a **2-axis 316 flat-bar X/Z cross-slide + a single Ruland USKC12-6-6-SS U-joint** (on a 304 corner plate) that absorbs the rigid-rotation arc travel and supplies the angular change — built from one shared `_corner_parts()` helper across every scene. **DYNAMIC COMPONENT** "Film Plane": click (Interact tool) → `ANIMATE("pose",0,1)` between **FLAT** (pose 0, vertical at mid-rail depth) and **TILT 20°/SWING 15°** (pose 1). The plane motion *is* a genuine rigid rotation, so the DC reproduces it **exactly** (single component, geometry direct, custom `pose` + same-component `_rotx_formula="20*pose"`/`_rotz_formula="15*pose"`, single `onclick`, `redraw_with_undo` AFTER commit). The **skate + cross-slide + U-joint travel WITH the plane** (built into the DC); the **U-channel rails stay static** — at the posed extreme the skate necessarily leaves the rail (the rigid-DC can't keep it on AND extend the slides; the default flat state reads seated). **The X/Z cross-slides are the NON-rigid part a DC can't animate**, so they're shown **statically with labels** in the non-interactive **"Corner detail (TR)"** scene (in-model `add_text` callouts on a `Labels` tag — see [[feedback-3d-model-labels]]). **Design note:** a rigid plane can't reach the old ±42°/±25.7° stops — at those angles a corner sweeps ~3.4 m of depth (through both end walls); practical envelope **tilt≤20°/swing≤15° combined**. **NB: DC interactivity is SketchUp-app only — does NOT carry to Sketchfab embeds; scene-tab cameras mis-frame in some builds → render via direct camera.** Reuses `ov.processing_tray()` + helpers; ghost-container context. **rev12:** the BR rail anchors via `ov.fp_combined_corner_plate` (shared with the right walkway, BR saddle skipped), and the **Combined view shows the IBC walkway cantilever arms** (`ov.ibc_cantilever_arms()` on a labeled `IBC Cantilever` tag — just the arms + upright clamps + bolts, NO IBC stack/frame). Reads `FP_*`, `RAIL_*`, `FP_ANGLE_LEG`, `PROC_TRAY_*`; the example DC pose (20°/15°) is **local** to the generator, but the envelope (`MAX_TILT_DEG`=40 / `MAX_SWING_DEG`=28) + cross-slide constants; the 2D `generate_film_plane_mechanism.py` sheets, the film-plane report, master-shopping-list and project-cost-breakdown carry the **built U-channel design + BOM (reconciled 2026-07)**. 3 scenes (Combined, No Container, Corner detail TR). 3D companion to 2D `generate_film_plane_mechanism.py`. |
| **film-plane-mechanism** | `src/models/generate_film_plane_mechanism_model.py` | `models/film-plane-mechanism.skp` + `src/models/film-plane-mechanism.rb` | The **built single-corner mechanism in fabrication detail** (renamed from `corner-gimbal` 2026-07; Sketchfab UID 572b4aaa…). One corner's full load path: 6061 angle frame → **304 SS corner plate** → **Ruland USKC12-6-6-SS U-joint** (input stub clamped to the X/swing slide by a **McMaster 4040N12 304 shaft support**; output stub to the plate by a countersunk cap screw) → **316 flat-bar X (swing) + Z (tilt) slides** → **acetal 4-wheel skate** (Ø32 wheels) → **304 U-channel rail**. Both rail pairs (bottom 4×2 web-vertical for load, top 3×1.5 flat for guide); left rail = **transport drop-in** (stub + welded bridge ON TOP + flush locating pin + bottom support bridge + pinhole gusset), right = flanged wall-to-wall. Top rail dropped 44mm (local `PZ_HB_TOP`) for 25mm ceiling clearance; corner plate raised to 25mm walkway clearance. Reads `tbs_constants` geometry (`C_HGT`, `RAIL_OFF_BOT`, `CD/CW_*`). **3D detail companion to 2D Sheets 3/4/5/8/9** — film-plane.skp carries the SAME corner design at whole-plane scale (split of responsibility: detail here, whole plane there). Scenes with per-part `add_text` labels. |
