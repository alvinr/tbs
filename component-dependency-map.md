...
| **film-plane** | `src/models/generate_film_plane_model.py` | `models/film-plane.skp` + `src/models/film-plane.rb` | Film plane + **4-corner tilt/swing mechanism**: the framed muslin screen posed in an example tilt+swing (corners parked at depths 2262/1962/1862/1562), and per-corner hardware at TL/TR/BL/BR (HGR20 rail + HGH20CA carriage + leadscrew + drive nut + universal joint + corner bracket). **Reuses** `ov.processing_tray()` (tray) + the `ov` helpers/scene scaffold; ghost-container context. Reads `FP_*`, `RAIL_*`, `FP_ANGLE_LEG`, `PROC_TRAY_*`. 6 scenes (Combined, Processing Tray, Corner TL/TR/BL/BR — each zoomed). Rebuild whenever the film-plane/rail/tray constants or `ov` change; 3D companion to 2D `generate_film_plane_mechanism.py`. |
| **lighttrap** | `src/models/generate_lighttrap_model.py` | `models/lighttrap.skp` + `src/models/lighttrap.rb` | Cargo-door end assembly (detailed): **housed revolving-door light lock** (fixed Ø900 housing with two 80° openings + single-opening C-shell drum, no fins, SKF 6215 bearings, drum↔housing seals, grab rail; light-tight by geometry), **hinged stepped panel** (3 zones, widened center + housing aperture + EPDM perimeter seal + 3 heavy-duty barrel hinges + 4 Southco cam latches), the **rev 9 / B2 punch-out bay** (`bay()` — a 4-wall ply tube that offsets the drum to X=−400, clearing the X=150 film-plane rail), **sliding carriage** (2× HGR20 ceiling rails + HGH20CA blocks + suspension brackets + left 60×60 SHS carriage beam + 4 Destaco toggle clamps), fixed RHS **door frame**, and **Fan B (intake)**. Fan B **reuses the Overview's shared `fan_duct()` builder** (same source as `fans()`), so re-run lighttrap whenever the fan changes. Reads `DRUM_*`, `PANEL_*`, `FAN_B_*`, `BAY_*` constants. Also carries the **shared cargo-door-end context** — a partial processing tray, near + far walkway decks, and the left film-plane rails — built by `processing_tray_partial()` / `walkways_partial()` / `film_plane_left()` (cropped to `PARTIAL_X`), the **same builders the transport model reuses** so both diagrams show identical details. Under B2 the left rail is **continuous in the operating pose** (the bay offsets the drum clear of it, so the old removable drum-zone segments are gone); for transport the whole rail is **struck** rather than gapped. 10 components, 7 scenes (Overview, Light-Trap Drum, Hinge Panel & Seal, Sliding Carriage, Fan B, Over Tray & Walkway, Film-Plane Rails (L)). Reuse seams for the transport model: `sliding_carriage(slide=0)`, `door_frame(include_seal=True)`, `context(left_walkway=True, x_far=None)` — all default to operating and byte-identical. |
| **lighttrap-transport** | `src/models/generate_lighttrap_transport_model.py` | `models/lighttrap-transport.skp` + `src/models/lighttrap-transport.rb` | The light trap posed in the **TRANSPORT position**: same details as **lighttrap** (whose builders it imports wholesale as `lt`), but the panel-mounted assembly — hinge panel + B2 punch-out bay + Ø900 housing + drum + EPDM seals + moving carriage parts + Fan B — is slid inward `PANEL_SLIDE` (~880mm, rev 9 / B2) on the rails so the bay retracts behind the cargo-door plane, locked at the transport Destaco clamp; the **closed ISO cargo doors** are added (GHOSTED, α≈0.2, so you see through them to the details) plus a **partial cargo-door-end context** — a cropped processing-tray section + the **near & far walkway decks** (left walkway removed for transport — `lt.context(left_walkway=False)` also drops the ghosted left-walkway deck baked into the operating context), reusing `ov` tray/walkway geometry. Both EPDM interfaces retract with the assembly: interface-1 panel-perimeter EPDM is in `hinge_panel()` (moves with the panel), and interface-2 **housing-surround EPDM** is built by `lt.housing_surround_seal()` into the **Light Trap** component so it retracts +880 with the housing (`lt.door_frame(include_seal=False)` omits it from the fixed frame; `include_seal=True` default keeps lighttrap byte-identical). The **film-plane rails are STRUCK for transport** (the panel slides past X=150) — `TAGS` drops the `Film Plane Rails` tag and the component is not built; the **Punch-Out Bay** (`lt.bay()`) is added instead. The container stub, tray, walkways and beams all crop to a common +X plane (`PARTIAL_X`=1600, passed to `lt.context(x_far=…)`) so their cut faces align. Fixed: container context, door frame, HGR20 rails, both Destaco lock-points, cargo doors, tray, walkways. **Rebuild whenever lighttrap or `PANEL_SLIDE` changes.** 10 components, 5 scenes (Overview, Transport — All, Over Tray & Walkway, Through the Doors, Light-Trap Drum). |
