...
| **spray-bar** | `src/models/generate_spraybar_model.py` | `models/spraybar.skp` + `src/models/spraybar.rb` | Spray-bar gantry detail — beam + housed ¾" LDPE pipe + 26 flat-fan nozzles (150mm pitch), wheel carriages (2 wheels/carriage, curved saddle axle clamps, top/bottom beam clamp plates), feed pole + ball joint, **processing tray** (floor + rim + sump). 5 scenes (Beam, Carriage Assembly, Pole & Ball Joint, Processing Tray, Combined). Reads `SPRAY_BAR_*` and `PROC_TRAY_*` constants. |
