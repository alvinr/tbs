...
| **ibc-stack** | `src/models/generate_ibc_model.py` | `models/ibc-stack.skp` + `src/models/ibc-stack.rb` | **v2 (`ibc-reconfig-v2`) — REBUILT 2026-06-14:** this is **THE canonical IBC 3D model** (the Sketchfab embed in `ibc-stacking-report.md`). Because it reuses the `ov` builders it now carries the full v2 design — `ov.ibc_rack()` = restraint FRONT PORTAL + exterior wall backing plates (was a load-bearing platform), `ov.equipment_panel()` pulled **forward** to the corridor mouth (film-safe, X>4649), and `ov.water_plumbing()` = Blue **side-entry** fill + the full recycle loop (sump→IBC-3, filter→IBC-2, reject→IBC-4, all side-entry near the top) + pumped X3/X4 drains. Re-sent + `.skp` re-saved (the `ibc_labels()` panel-equipment anchors were moved forward to X≈4814–4854 to track the panel). (The standalone **`ibc-v2`** study below is now redundant.) IBC tote stacking arrangement at the IBC end. Reuses Overview builders: `ov.ibc_stack()` (4× 1000L totes), `ov.ibc_rack()` (v2 restraint front portal), `ov.equipment_panel()` (pump/filter), `ov.water_plumbing()`, `ov.water_hookups()` (exterior wall). Container is a low-alpha ghost. 5 tags (Context / IBC Tanks / IBC Frame / Plumbing & Panel / **Walkway Cantilever**) + a `Labels` tag. The **Walkway Cantilever** component (`ov.ibc_cantilever_arms()` — the 2 right-walkway support arms + upright clamps + M12 bolts that attach to the corridor uprights, rev12; single-sourced with the overview/walkway models). **5 scenes:** IBC Tanks, **IBC Frame** (the frame + the attached walkway cantilever arms — the `Walkway Cantilever` tag rides in this scene), Plumbing & Panel, Combined, **Labeled** (`ibc_labels()` — `add_text` callouts on the `Labels` tag, shown only in this scene; 19 callouts split by side. **Container/front side:** the near-column totes (BROWN developer + BLUE #1), the **SUMP PICKUP** riser, the **TO SPRAY BAR** Blue feed, **IBC FRAME**, and the **wet-end panel equipment** pulled out the front laid out as the panel reads — two pump columns (left ACC-01/P-04/P-01, right P-05/P-03/P-02) + three filters F1/F2/F3 below (pumps/filters/accumulator only, no pipes/valves). **Sealed-end side** (right-hand column): the far-column totes (WASTE + BLUE #2) plus the exterior bulkhead ports **X1** (fresh fill), **X3** (Brown drain-out), **X4** (Waste drain-out). Per the project rule every .skp gets a Labeled scene — see [[feedback-3d-model-labels]]). Rebuild whenever those `ov` builders or the IBC/plumbing constants change. |
| **ibc-v2** | `src/models/generate_ibc_v2_model.py` | `src/models/ibc-v2.rb` (`.skp` never saved) | **⚠ REDUNDANT (2026-06-14) — candidate for retirement.** Built as the v2 IBC study while `ibc-stack` still showed the old design; now that `ibc-stack` (above) is rebuilt to v2 it is the canonical model and this duplicate is superseded. Self-contained but reuses `ov` primitives + colors. Builders: `context()` — full-length ghost container **plus a translucent film-rail NO-GO plane at X=4649** (the swept volume that forces all equipment to X≥4674); `ibc_totes()` — **Config-1** direct-stack (near col Brown+Blue#1, far col Waste+Blue#2) at 1000L (`IBC_H_1000`/`IBC_H_STK_1000`) with wall-hanger plates/seats; `ibc_frame()` — the **single FRONT PORTAL** at `RWK_X_UP`=4734 (uprights + top tie + floor beam + panel-mount rail + **Front Retaining Bar** `FRONT_X−20` 50×20 in the 25mm gap, clear of totes ≤4674 + film rail ≥4649 + front-bar stubs + **D-ring holders** + floor feet); `plumbing_firstpass()` — X1 Blue side-fill, **X3 Brown / X4 Waste pumped drains** (gravity insufficient). Reuses `ov.right_walkway_cantilever()` (the proven bottom-arm cantilever, carried by the portal uprights), `ov.equipment_panel()` **translated forward** by `PANEL_FWD_DX=−366` (corridor mouth, film-safe), `ov.water_hookups()` (X1/X3/X4 exterior). Tags: Context / IBC Tanks / IBC Frame / Walkway / Panel / Pipes / Labels. **6 scenes:** IBC Tanks, IBC Frame + Walkway, Forward Panel, Plumbing, Combined, Labeled. `--save` writes `ibc-v2.rb`; `--send` builds into the ACTIVE blank doc (NOT overview). If kept, rebuild whenever the v2 IBC/frame/plumbing constants or the reused `ov` builders change. |
