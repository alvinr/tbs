...
| **film-plane** | `src/models/generate_film_plane_model.py` | `models/film-plane.skp` + `src/models/film-plane.rb` | Film plane + **4-corner tilt/swing mechanism**. The framed muslin screen is a **DYNAMIC COMPONENT** ("Film Plane"): click with the Interact tool → `ANIMATE("pose", 0, 1)` toggles it between **FLAT** (pose 0, neutral plane at Y=FP_Y) and the example **TILT+SWING** (pose 1). Since the real motion is a non-rigid 4-corner warp a DC can't deform, the DC is the equivalent RIGID rotation about the fixed TL corner — `_rotx_formula="TILT*pose"` (≈9.5° tilt, bottom forward) + `_rotz_formula="SWING*pose"` (≈−3.8° swing, right forward); built flat in LOCAL coords, instanced at the TL pivot (150, 2262, 2388), registered via `$dc_observers…redraw_with_undo`. Per-corner hardware at TL/TR/BL/BR (HGR20 rail + HGH20CA carriage + leadscrew + drive nut + universal joint + corner bracket) stays **static context** at the posed depths (2262/1962/1862/1562). **NB: DC interactivity is SketchUp-app only — it does NOT carry to Sketchfab embeds.** **Reuses** `ov.processing_tray()` + the `ov` helpers/scene scaffold; ghost-container context. Reads `FP_*`, `RAIL_*`, `FP_ANGLE_LEG`, `PROC_TRAY_*`. 6 scenes (Combined, Processing Tray, Corner TL/TR/BL/BR — each zoomed). Rebuild whenever the film-plane/rail/tray constants or `ov` change; 3D companion to 2D `generate_film_plane_mechanism.py`. |
