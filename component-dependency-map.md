...
| **lighttrap** | `src/models/generate_lighttrap_model.py` | `models/lighttrap.skp` + `src/models/lighttrap.rb` | Cargo-door end assembly (detailed): **housed revolving-door light lock** (fixed Ø900 housing with two 80° openings + single-opening C-shell drum, no fins, SKF 6215 bearings, drum↔housing seals, grab rail; light-tight by geometry), **hinged stepped panel** (3 zones, widened center + housing aperture + EPDM perimeter seal + 3 piano hinges + 4 Southco cam latches), **sliding carriage** (2× HGR20 ceiling rails + HGH20CA blocks + suspension brackets + left 60×60 SHS carriage beam + 4 Destaco toggle clamps), fixed RHS **door frame**, and **Fan B (intake)**. Fan B **reuses the Overview's shared `fan_duct()` builder** (same source as `fans()`), so re-run lighttrap whenever the fan changes. Reads `DRUM_*`, `PANEL_*`, `FAN_B_*` constants. Also carries the **shared cargo-door-end context** — a partial processing tray, near + far walkway decks, and the left film-plane rails — built by `processing_tray_partial()` / `walkways_partial()` / `film_plane_left(demount=True)` (cropped to `PARTIAL_X`), the **same builders the transport model reuses** so both diagrams show identical details. In the operating pose the two removable drum-zone rail segments (Yd 731–1631, upper + lower) are shown **in place in the demountable amber `ov.C_DEMOUNT`** — the same marker the overview 3D model uses (the 2D film-plane diagrams use red); `film_plane_left(demount=False)` (transport) takes them out as a gap. 9 components, 7 scenes (Overview, Light-Trap Drum, Hinge Panel & Seal, Sliding Carriage, Fan B, Over Tray & Walkway, Film-Plane Rails (L)). Reuse seams for the transport model: `sliding_carriage(slide=0)`, `door_frame(include_seal=True)`, `context(left_walkway=True, x_far=None)` — all default to operating and byte-identical. |
| **lighttrap-transport** | `src/models/generate_lighttrap_transport_model.py` | `models/lighttrap-transport.skp` + `src/models/lighttrap-transport.rb` | The light trap posed in the **TRANSPORT position**: same details as **lighttrap** (whose builders it imports wholesale as `lt`), but the panel-mounted assembly — hinge panel + Ø900 housing + drum + EPDM seals + moving carriage parts + Fan B — is slid inward `PANEL_SLIDE` (550mm) on the rails so the housing clears the cargo-door plane, locked at the transport Destaco clamp; the **closed ISO cargo doors** are added (GHOSTED, α≈0.2, so you see through them to the details) plus a **partial cargo-door-end context** — a cropped processing-tray section + the **near & far walkway decks** (left walkway removed for transport — `lt.context(left_walkway=False)` also drops the ghosted left-walkway deck baked into the operating context), reusing `ov` tray/walkway geometry. Both EPDM interfaces retract with the assembly: interface-1 panel-perimeter EPDM is in `hinge_panel()` (moves with the panel), and interface-2 **housing-surround EPDM** is built by `lt.housing_surround_seal()` into the **Light Trap** component so it retracts +550 with the housing (`lt.door_frame(include_seal=False)` omits it from the fixed frame; `include_seal=True` default keeps lighttrap byte-identical). Also adds a **left-end film-plane rail partial** (`film_plane_left()`): the upper (TL) + lower (BL) left corner rails at `RAIL_X_L`, the brace-cage beams + corner posts tying them at the near-/far-wall ends, with **both left rails' demountable drum-zone segments (Yd 731–1631) taken out** (a gap in each) per the 2D film-plane diagrams. The container stub, tray, walkways and beams all crop to a common +X plane (`PARTIAL_X`=1600, passed to `lt.context(x_far=…)`) so their cut faces align. Fixed: container context, door frame, HGR20 rails, both Destaco lock-points, cargo doors, tray, walkways, film-plane rails. **Rebuild whenever lighttrap or `PANEL_SLIDE` changes.** 10 components, 6 scenes (Transport — All, Over Tray & Walkway, Film-Plane Rails (L), Through the Doors, Light-Trap Drum). |
