...
| **walkway** | `src/models/generate_walkway_model.py` | `models/walkway.skp` + `src/models/walkway.rb` | Processing-tray **perimeter walkway + how it's held up**, separated so the structure reads apart from the decks. 5 tagged components: **Walkways** (the 4 removable grated "gate" sections — near + widened, far, right, left + drum-exit punch-out); **Cantilevers** (near/far wall-cantilevered gusset brackets carrying the near/far decks, with the **exterior** detail — reinforcing plate + through-bolts, hex heads outside; standard brackets 8mm/150mm-leg/300mm-arm with **3× M12** triangular, the four widened EP/battery-zone brackets (near, X 1155–2629) 10mm/200mm-leg/500mm-arm with **4× M12** rectangular — matching Sheet 7); **Right Hangers** (ceiling-hung right walkway — 2 bearer angles + 5 rod-pairs of M10 rod to ceiling plates); **Left Support** (removable lift-out — a **full-width steel edge beam** on **bolt-through wall seats** [IBC-style, see [`spec`](docs/superpowers/specs/2026-06-05-left-walkway-edge-beam-support.md)] + bearing strip + 3 floor legs); **Processing Tray** (reuses `ov.processing_tray()`). Container is a low-alpha ghost so the exterior braces + bolt-throughs show. 6 scenes separating gates / cantilevers / right hangers / left support / tray. Reads `WALKWAY_*`, `PROC_TRAY_*`, `LEFT_WK_*`. The edge-beam-on-wall-seats design is fully propagated to the 2D `generate_walkway_diagram.py` (Sheets 1/4/5/6/9), walkway-report §5, master-shopping-list, and project-cost-breakdown. |
| **film-plane** | `src/models/generate_film_plane_model.py` | `models/film-plane.skp` + `src/models/film-plane.rb` | Film plane + **4-corner tilt/swing mechanism**. The framed muslin screen is a **DYNAMIC COMPONENT** ("Film Plane"): click with the Interact tool → `ANIMATE("pose", 0, 1)` toggles it between **FLAT** (pose 0, neutral plane at Y=FP_Y) and the example **TILT+SWING** (pose 1). Since the real motion is a non-rigid 4-corner warp a DC can't deform, the DC is the equivalent RIGID rotation about the fixed TL corner. **Structure = the proven cargo-door pattern:** a PARENT "Film Plane" holds `pose` + `onclick=ANIMATE("pose",0,1)`, placed at the TL pivot (150, 2262, 2388); a NESTED CHILD "Film Plane Screen" (flat geometry in LOCAL coords) carries `_rotx_formula="9.51*FilmPlane!pose"` (tilt, bottom forward) + `_rotz_formula="-3.81*FilmPlane!pose"` (swing, right forward). The child-reads-parent formula updates during the parent's animation; a **same-component** pose→rotx formula does NOT (the click silently no-ops — first attempt's bug). Registered via `$dc_observers…redraw_with_undo`. Per-corner hardware at TL/TR/BL/BR (HGR20 rail + HGH20CA carriage + leadscrew + drive nut + universal joint + corner bracket) stays **static context** at the posed depths (2262/1962/1862/1562). **NB: DC interactivity is SketchUp-app only — it does NOT carry to Sketchfab embeds.** **Reuses** `ov.processing_tray()` + the `ov` helpers/scene scaffold; ghost-container context. Reads `FP_*`, `RAIL_*`, `FP_ANGLE_LEG`, `PROC_TRAY_*`. 6 scenes (Combined, Processing Tray, Corner TL/TR/BL/BR — each zoomed). Rebuild whenever the film-plane/rail/tray constants or `ov` change; 3D companion to 2D `generate_film_plane_mechanism.py`. |
| **film-plane-optA** | `src/models/generate_film_plane_optA_model.py` | `models/film-plane-optA.skp` + `src/models/film-plane-optA.rb` | **Option A tilt/swing mechanism** — the chosen direction (2026-06-06). The film is a **FIXED-SIZE rigid rectangle** (FP_W×FP_H) that only changes ANGLE; each corner's existing **HGR20 rail + leadscrew** (depth/focus, unchanged) gains a **2-axis X-Z cross-slide + spherical rod-end** that absorbs the rigid-rotation arc travel. Per-corner chain: rail → carriage + drive nut → **X cross-slide (blue = SWING float)** → **Z cross-slide (green = TILT float)** → rod-end → frame corner. Posed at a **realistic tilt 20° / swing 15°** where all four carriages stay on the 2200 mm design rail (corner depths 204..2158, inside 100..2262). **Design note:** a rigid plane can't reach the old ±42°/±25.7° stretch-mechanism stops — at those angles a corner sweeps ~3.4 m of depth (through both end walls); practical envelope is **tilt≤20°/swing≤15° combined**. 2 components (Mechanism, Plane) + a **Labels** tag (in-model `add_text` callouts with leaders, shown only in the corner-detail scene — see [[feedback-3d-model-labels]]). 2 scenes (Assembly, Corner detail TR). Reads `FP_*`, `RAIL_*`, `FP_ANGLE_LEG`; pose/cross-slide numbers are **local** to the generator (NOT yet in `tbs_constants` — phased: 3D first, 2D diagrams/reports/costs deferred). **NB:** scene-tab cameras mis-frame in some SketchUp builds → render via direct camera + `write_image`. **Supersedes the mechanism in `film-plane`** (the old stretching/U-joint DC) pending the full 2D+report cascade. |
| **lighttrap** | `src/models/generate_lighttrap_model.py` | `models/lighttrap.skp` + `src/models/lighttrap.rb` | **Single interactive cargo-door-end model** (rev 9 / B2 — consolidates the former separate operating + transport models into one .skp via Dynamic Components). **Static parts:** ghosted container stub (ends at the door plane X=0 so the bay/drum/doors read as protruding beyond it), fixed RHS **door frame** (`door_frame(include_seal=False)` — the housing-surround EPDM rides the moving housing), fixed **carriage rails + Destaco locks** (`carriage_fixed()`), partial **processing tray**, near/far **walkways**, and the continuous left **film-plane rails**. **Two Dynamic Components** (click with SketchUp's Interact tool): **(1) Panel Slide** — the moving assembly (hinged stepped panel + 3 barrel hinges + 4 Southco latches, B2 **punch-out bay** `bay()` offsetting the drum to X=−400, the **housed revolving-door light lock** Ø900 housing + C-shell drum + seals + grab rail, **Fan B**, and the moving carriage blocks/brackets/beam via `carriage_moving()`) animates X between operating (0) and transport (`TRANSPORT_SLIDE`≈880mm); **(2) Cargo Doors** — a parent DC whose `shut` attribute drives two leaf children's `RotZ` (`door_leaf_local()`), swinging both ISO leaves 0↔±180° (closed↔open). Fan B **reuses the Overview's shared `fan_duct()`**. Reads `DRUM_*`, `PANEL_*`, `FAN_B_*`, `BAY_*`, `PANEL_SLIDE`. **Requires the Dynamic Components extension** for the click behavior — it does **not** carry to Sketchfab web embeds (those render a static pose). 8 components (6 static + 2 DCs), 1 scene. **Rebuild whenever those constants or the shared builders change.** |
