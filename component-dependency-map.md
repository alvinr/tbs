...
| **lighttrap-transport** | `src/models/generate_lighttrap_transport_model.py` | `models/lighttrap-transport.skp` + `src/models/lighttrap-transport.rb` | The light trap posed in the **TRANSPORT position**: same details as **lighttrap** (whose builders it imports wholesale as `lt`), but the panel-mounted assembly — hinge panel + Ø900 housing + drum + EPDM seals + moving carriage parts + Fan B — is slid inward `PANEL_SLIDE` (550mm) on the rails so the housing clears the cargo-door plane, locked at the transport Destaco clamp; the **closed ISO cargo doors** are added (GHOSTED, α≈0.2, so you see through them to the details) plus a **partial cargo-door-end context** — a cropped processing-tray section + the far walkway deck (near walkway omitted for the view; left walkway removed for transport), reusing `ov` tray/walkway geometry. Fixed: container context, door frame, HGR20 rails, both Destaco lock-points, cargo doors, tray, walkway. **Rebuild whenever lighttrap or `PANEL_SLIDE` changes.** 9 components, 5 scenes (Transport — All, Over Tray & Walkway, Through the Doors, Light-Trap Drum). |
