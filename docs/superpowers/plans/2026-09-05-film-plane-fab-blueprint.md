...
4. **Render:** `python3 src/generators/generate_film_plane_mechanism.py` — confirm the new PNG writes and the run ends "Done." with no traceback.
5. **Tidy labels:** `python3 src/generators/tidy_labels.py --fix diagrams/film-plane-sheetN.png` (static pass), then open the PNG and do a crop-zoom visual pass per `skills/skill_tidy_labels.md` — no leader crossing a solid, no label off its panel, all dims carry `mm`.
6. **Verify vs 3D (read-only):** query the matching component's bounds/positions in the live `film-plane-mechanism.skp` via `eval_ruby` and confirm the sheet's key dimensions match. If the live doc is not that model, DO NOT touch it — note the mismatch and move on (the constants are the source of truth regardless).
