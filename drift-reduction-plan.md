...
| **4 dependency** | **missing-cascade** check; ✅ **hardwired-literal scan** (`lint.py --literals` full scan + staged-scoped warning; HIGH re-declarations + non-round bare-literal matches); ✅ **12-item backlog cleared** (all local copies now import/reference the constant; scan reports 0) | duplication-equality checks; formal `dependencies` data |
