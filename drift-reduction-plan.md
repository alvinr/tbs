...
- ⚠ **walkway bracket** (`_cantilever_parts()` ↔ `walkway_brackets()`) — **DIVERGED** (the documented 2026-06-06 split): the dedicated walkway model emits the exterior reinforcing plates (6×100×180, 6×120×220), hex heads (6×18×18) and full-length M12 through-bolts (cyl r6 × 54/56); the overview's inline `walkway_brackets()` omits the reinforcing plates and models short interior studs (cyl r6 × 20/22). **Likely an intentional level-of-detail choice** (whole-system overview vs. focused fab model), not a dimension error — so this pair is **on-demand only** (`--duplication`), NOT a commit warning, pending a human call to either single-source it or accept the LOD gap.
