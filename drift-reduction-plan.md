...
| **4 dependency** | **missing-cascade** check; ✅ **hardwired-literal scan** (`lint.py --literals` + staged warning; 12-item backlog cleared, scan = 0); ✅ **duplication-equality check** (`lint.py --duplication` + staged warning for precise pairs — monkeypatches the geometry helpers, calls both emitters, compares what they build) | reconcile the walkway-bracket LOD divergence (or accept it); formal `dependencies` data |
