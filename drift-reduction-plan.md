...
| **4 dependency** | **missing-cascade** check (now output-targeted); ✅ **hardwired-literal scan** (12-item backlog cleared, scan = 0); ✅ **duplication-equality check** (`--duplication`); ✅ **`dependencies.yml`** — the single structured `script → output` graph (`deps.py` loads it; `deps_valid` self-checks it vs the filesystem + each script's writes; missing-cascade + `--cascade <CONST>` consume it; the `.md` §4 table was retired into the computed view) | — (Phase 4 complete) |

**Duplication-equality findings (the "NB: keep in sync" pairs, checked by calling both emitters):**
- ✅ **electrical EP core** (`power_core()` ↔ `electrical()`) — **IN SYNC**. Compared per canonical part-key in the shared world frame (enclosure, MPPT, fuse-block + 7 fuses, busbars, disconnect, main feed, **PV feed**); every shared part emits byte-identical geometry. This is a PRECISE check (same frame) → wired as a staged-scoped commit warning.
- ✅ **walkway bracket** (`_cantilever_parts()` ↔ `walkway_brackets()`) — **ACCEPTED LOD DIFFERENCE** (signed off 2026-06): the dedicated walkway model is the full-fab version (exterior reinforcing plates 6×100×180 / 6×120×220, hex heads, full-length M12 through-bolts cyl r6 × 54/56); the overview deliberately simplifies it for the whole-system view (omits the reinforcing plates, models short interior studs cyl r6 × 20/22) — the same convention as the 2D thin-section scale exaggeration. The difference is level-of-detail only, **not** the load-bearing dimensions. Recorded in both function docstrings + the `_DUP_PAIRS` `accepted` note; `--duplication` lists it as **EXPECTED**, and it never warns. (If the overview is ever meant to show full fab detail, single-source it by importing `_cantilever_parts()`.)
