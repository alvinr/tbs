...
**Disconnected endurance (no AC charge, solar top-up only): the system is clean-water limited, not power limited.** With sun it is solar-positive (+1,300 Wh/day at 3 prints) → runs **indefinitely on either 1 or 2 packs**; the fresh Blue supply (1,600 L / 121 L net per print) caps a deployment at **~13 prints ≈ 4.4 days @ 3/day**. Battery count sets only the *cloudy-day reserve* (1 pack ≈ 0.6 day, 2 packs ≈ 1.2 day) and the 4-print-day headroom — **not** the deployment length. The Black waste tote (1,000 L) is a parallel out-flow limit for fully self-contained field use.
