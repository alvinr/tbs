...
Swing is the binding limit because the plane is <!-- BEGIN fact:film_plane_width_mm -->4,389<!-- END fact:film_plane_width_mm -->mm wide — the same depth travel over a wider span sweeps the corner further along the rail. Combined tilt+swing is further limited — see §5.
...
- **Travel** — the Z (tilt) and X (swing) strokes = (half-dimension) × (1 − cos angle), rotating about the plane center: Z = (FP_H/2)(1 − cos 40°) = **245 mm** and X = (FP_W/2)(1 − cos 28°) = **257 mm**, matching `XSLIDE_Z_TRAVEL` / `XSLIDE_X_TRAVEL` exactly.
- **Load** — the ~51 kg moving plane puts ~W/4 = **124 N** on each corner. The governing case is the horizontal X (swing) slide in bending at full extension (the Z slide takes gravity axially). **The cross-slide bars are mounted deep** — the 38.1 mm dimension in the load direction — giving σ ≈ 21 MPa, **safety factor ≈ 10**, deflection ≈ 0.1 mm. Mounted flat the bar is marginal (SF ≈ 1.7, failing under a 2× dynamic/asymmetry factor), so **deep mounting is specified**.
...
The color assembly/mechanism views come first; the monochrome single-part dimensional layouts close the set (Sheets 15–18 are the pure single-part fab sheets).

- **Sheet 12 — Corner assembly:** the exploded build sequence and the full corner fastener schedule.
- **Sheet 13 — Depth rail:** 3×1½" 6061-T6 U-channel (Grainger 795M51), cut wall-to-wall from 8 ft stock, section + fixed/drop-in end conditions.
- **Sheet 14 — Acetal skate:** the axle-saddle retention detail + the 6061 carriage-plate hole coordinates.
- **Sheet 15 — Cross-slide stack:** the Z (tilt) and X (swing) 304 flat bars, the deep-mount section (SF≈10 vs 1.7 flat), gib, and stroke.
- **Sheet 16 — Corner angle:** the stock 4×4×¼" 304 angle drilling layout with hole coordinates, and the L-section — the U-joint mount.
- **Sheet 17 — Frame weldment:** the 2×2×⅛" 6061 angle frame to scale, the 4 corner welds (the only joints), and the member section with ACM backing.
...
| [3/8" 304/304L SS rod — U-joint stub shafts (1× 3 ft)](https://www.mcmaster.com/89535K87/) (89535K87) | Input + output stub shafts into the U-joint (2/corner ×4 = 8 short stubs, ~60–80mm each ≈ 560–640mm + kerf). ONE 3 ft (914mm) length ($13.25 firm) yields all 8 with margin. Each stub gets a 3/32×3/64 KEYSEAT for the SSNBUJ750x3/8KB keyway bore (fp-ujoint-key) — keyed for anti-rotation, then locked axially by the joint set screw (J3/J4). | 1 lot | McMaster-Carr | $13 |
...
| [1/8×3/4in 304 flat-bar axle saddles (skate) — 2 ft bar](https://www.mcmaster.com/8992K794-8992K797/) (8992K794) | Axle retention — formed from 1/8" (3.18mm) × 3/4" (19mm) 304 SS flat bar, wrapped over each Ø10 axle (1mm cradle clearance) with two ~12mm feet bolted through the carriage plate (2× Ø6.5 M6). One saddle per axle, 4/skate ×4 = 16; 8 cut per 2 ft (610mm) length → 2 bars. Same stock + method as the spray skate. $9.84/2 ft bar (confirmed 2026-09-06). | 2 2 ft bar | McMaster-Carr | $20 |
| [M6×1.0 × 20 hex bolt, 304 SS (A2-70)](https://www.mcmaster.com/91287A137/) (91287A137) | Skate axle-saddle retention — 2× M6 per saddle, 16 saddles = 32. Grip = 6mm carriage plate + 3.18mm saddle foot ≈ 9.2mm → M6×20 (nyloc under the plate). 304 SS A2-70 (wet cyanotype zone; no chloride → 316 unneeded). Consolidated onto the shared M6×20 — retires the former M5 (one M6 family, fastener-standardization). McMaster 91287A137 $17.77/pack of 50 firm. | 32 ea | McMaster-Carr | $11 |
| [M6×1.0 hex nut, nyloc SS](https://www.mcmaster.com/90576A115/) (90576A115) | Nyloc nut under the carriage plate — M6×20 skate axle-saddle screws (2/saddle ×16 = 32). Transport-vibration-safe. Pitch M6×1.0 coarse — 90576A115. $4.77/pack of 100. | 32 ea | McMaster-Carr | $2 |
...
| [M8×1.25 × 25 hex bolt, 304 SS (A2-70) — cross-slide joints J1/J2](https://www.mcmaster.com/92314A711/) (92314A711) | J1 (skate carriage → Z-slide) + J2 (X-way → Z-carriage) — 4× M8×25 each, 2 joints/corner ×4 = 32. THROUGH-BOLT + M8 nut (flat washer under head, split under nut), 24 N·m, Loctite 243 (Sheet 8). Length M8×25 for the 14mm J1 grip (6mm carriage plate + 8mm Z-carriage wall) + 2 washers + nut. 304 SS A2-70 (wet cyanotype zone; no chloride → 316 unneeded). McMaster 92314A711 $10.60/pack of 25 (sourced 2026-09-06). | 32 ea | McMaster-Carr | $14 |
| [M8 flat washer, 304 SS](https://www.mcmaster.com/98689A673/) (98689A673) | Flat washer under each J1/J2 M8×20 head (Sheet 8). 304 SS, wet zone. McMaster 98689A673 $13.59/pack of 50 (sourced 2026-09-06). | 32 ea | McMaster-Carr | $9 |
| [M8 split lock washer, 304 SS](https://www.mcmaster.com/91111A132/) (91111A132) | Split lock washer under each J1/J2 M8×20 (with Loctite 243 — belt + suspenders vs transport vibration; Sheet 8). 304 SS, wet zone. McMaster 91111A132 $11.19/pack of 100 (sourced 2026-09-06). | 32 ea | McMaster-Carr | $4 |
...
| [M4×0.7 × 12 hex bolt, 304 SS — cam-clamp base mount](https://www.mcmaster.com/91280A134/) (91280A134) | Cam-clamp base → mount tab, BOLT-THROUGH (bolt + nut, not tapped) — 2× M4×0.7 per clamp, 3 clamps/corner ×4 = 24. The 5128A63 base has 2× M4×0.7 holes at ~15.7mm spacing. 304 SS (wet cyanotype zone). McMaster 91280A134 $11.18/pack of 100 (sourced 2026-09-06). | 24 ea | McMaster-Carr | $3 |
| [M4×0.7 hex nut, 304 SS](https://www.mcmaster.com/91828A231/) (91828A231) | Plain hex nut under the cam-clamp mount tab — M4×12 bolt-through (2/clamp ×12 = 24). 304 SS, wet zone. Pitch M4×0.7. McMaster 91828A231 $7.25/pack of 100 (sourced 2026-09-06). | 24 ea | McMaster-Carr | $2 |
...
| [Stainless self-drilling (TEK) screw — ACM → 6061 frame](https://www.mcmaster.com/90599A716/) (90599A716) | Secures the ACM backing to the 6061 perimeter frame leg, driven ABOVE the frame's inside fillet ridge (Sheet 17 View C). Stainless self-drilling — drills the 3.2mm 6061 leg + the 3mm Dibond, no pilot. ~300mm pitch along the top + 2 side edges (~8.6 m; bottom edge = walkway/swing clearance) → ~29 + spares. Stainless for the wet cyanotype splash zone; the head sits on the frame BACK (not the image face). McMaster 90599A716 $6.46/pack of 10 (sourced 2026-09-06) → 3 packs = 30. | 30 ea | McMaster-Carr | $19 |
...
**Transport mode.** The film-plane left rail is **continuous** (no demountable center segment): the light lock (Ø800 housing + drum) is offset (`DRUM_CX = −400`) and exits through the hinge-panel punch-out bay rather than rotating within the rail span. For *transport*, the panel + drum **SWING ~56°** about the pivot and the drum cage transitions X=260, so the **two left film rails (TL + BL) and the muslin screen are struck first** — the left rails lift out of their thumb-screw saddles and re-seat to the film datum on re-deployment — see [Hinged Panel Report](hinged-panel-report.md) §5.4 for the conversion sequence.
