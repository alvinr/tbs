...
leader(ax, sx(carriage_cx), sz(WHEEL_AXLE_Z - WHEEL_DIA / 2 - 5),
       sx(carriage_cx - 80), sz(WHEEL_AXLE_Z - WHEEL_DIA / 2 - 35),
...
draw_dim_v(ax, sx(wk_r - 20), sz(BEAM_Z_TOP), sz(GRATE_Z_BOT),
           f"{gap_to_grate:.0f}mm\nCLR", offset=3 / H_SC, fs=4.5, font=FONT)

draw_dim_v(ax, sx(carriage_cx + 50), sz(TRAY_FLOOR_Z), sz(TRAY_FLOOR_Z + WHEEL_DIA),
           f"Ø{WHEEL_DIA}mm", offset=3 / H_SC, fs=5, font=FONT, right=True)

# ── Scale note ────────────────────────────────────────────────────────────
ax.text(sx(X_LO + 20), sz(Z_HI - 5),
        f"HORIZONTAL SCALE 1:{int(H_SC)} / VERTICAL SCALE 1:{V_SC:.1f}\n"
        "VERTICAL EXAGGERATED 4× FOR CLARITY",
        ha="left", va="top", fontsize=5.5, color=C_DIM, style="italic",
        **FONT, zorder=15)

# ── Notes ─────────────────────────────────────────────────────────────────
notes = [
    "SPRAY BAR — GANTRY CONCEPT:",
    f"1. 40×40×3mm AL SHS doubles as beam AND spray pipe (water through bore).",
    f"2. Carriages (2× Ø{WHEEL_DIA}mm nylon wheels) ride on tray floor under grating.",
    f"3. Beam spans {BEAM_SPAN}mm (X dir). Coverage: X={PROC_OPEN_X_L}–{PROC_OPEN_X_R}mm.",
    f"4. Travel: {SPRAY_BAR_TRAVEL}mm along Yd. Tray rim walls guide laterally.",
    "5. Both ends capped; hose connects at one end. RIGHT CARRIAGE MIRRORS LEFT.",
]
draw_notes(ax, notes, sx(X_LO + 20), sz(Z_HI), spacing=8 / V_SC,
           fs=4.5, font=FONT, width=6000)
...
leader(ax2, dy(wheel1_yd - WHEEL_DIA / 2 - 3), dz(WHEEL_AXLE_Z),
...
leader(ax2, dy(plate_yd_r + 3), dz(WHEEL_AXLE_Z),
...
draw_dim_v(ax2, dy(beam_yd_l - 15), dz(BEAM_Z_BOT), dz(BEAM_Z_TOP),
           f"{BEAM_W}mm", offset=3 / SC2, fs=5.5, font=FONT)

draw_dim_h(ax2, dy(wheel1_yd), dy(wheel2_yd), dz(TRAY_FLOOR_Z + WHEEL_DIA + 8),
           f"{WHEEL_SPACING_YD}mm", offset=3 / SC2, fs=5.5, font=FONT)

draw_dim_v(ax2, dy(DET_YD_LO + 12), dz(GRATE_Z_BOT), dz(GRATE_Z_TOP),
           f"{WALKWAY_GRATE_T}mm", offset=3 / SC2, fs=5, font=FONT)
