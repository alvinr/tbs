...
The entire panel slides 300mm in the X direction on HGR20 linear rails mounted to both container walls at floor and ceiling level (4 rails, 8 carriage blocks). The left (hinge) side rides via a vertical carriage beam (60×60×3mm SHS, 2,400mm tall); the right (latch) side rides directly on blocks attached to the panel frame. For transport, the waste drums slide inward 305mm on V-groove roller dollies (using removable bridge track sections that span the film plane floor rail), then the panel slides inward 300mm — clearing the container doors for closure. Single-person operation, 15–20 minutes per mode conversion. See [Equipment Layout Report](equipment-layout-report.md) §6.1 for dolly track specification and §6.2 for sliding carriage specification.
...
- **Transport mode conversion** (single person, 15–20 minutes): retract film plane carriage → install bridge track sections → release cam latches → swing panel open → slide waste drums inward 305mm → remove bridge sections → swing panel closed → slide panel inward 300mm → lock → close container doors. See [Equipment Layout Report](equipment-layout-report.md) §6 for full specification.
