...
- The drum is installed into the hinged panel before the panel is hung. Panel + drum combined weight: approximately **279 kg** (first-principles, plastic-skin housing/drum ~60 kg + steel-framed stepped panel with 3mm-aluminum corner plates + B2 bay/pivot hardware ~32 kg) — down only slightly from ~286 kg with the rev 8 aluminum shells, because the ~39 kg saved on the shells is largely offset by the added B2 bay + pivot post / bearings. Still requires an engine crane or gantry hoist for hanging (beyond a two-person lift).
