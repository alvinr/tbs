...
- [ ] 0.5.4 — Install perimeter walkway grating sections onto wall-cantilevered brackets around all 4 sides of the processing tray. Near, far, and right grating panels lift onto bracket arms and are secured with grating clips. The left walkway (cargo door end) is a removable lift-out section — it rests on 5 floor-leg cantilever brackets bolted to the bare floor outside the tray (arms reach X=470, three extended to X=770 on the drum-exit punch-out; the hinged panel occupies this end wall). Left corners use butt joints (no miter) so near/far walkways start at X=470, entirely past the panel transport envelope. The left walkway grate must be removed before sliding the panel to transport position; the floor brackets stay bolted. No contact with the tray (brackets sit on bare floor outside it).
