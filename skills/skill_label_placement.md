...
1. **Render it and crop-zoom every label cluster.** Never commit a diagram you have only reasoned about — look at the pixels (PIL crop at 2.5–3×). Most of my misses this week (floor-flange "keep it long," spray-bar anchor) came from reasoning instead of looking. **First run `tidy_labels.py --overflow <generator>`** — it renders every sheet and mechanically lists any label off the frame edge or overlapping another, so the crop-zoom is spent on judgement, not hunting; resolve every OVERFLOW before you ship.
