...
### Learned from a re-review of the hingepanel "Tidy labels" commits (2026-06-12)

66. **Shape a label's own line-wrapping to the whitespace around it — split a line to go narrow-and-tall where it's horizontally crowded; join lines to go wide-and-short where there's horizontal room.** Repositioning a label is rule 13/35; this is the complementary move of re-wrapping the *same words* so the text block's footprint matches the available gap. The user did this in both directions in one pass: near the packed panel face a one-line parenthetical became two lines (`"(U-channel set; flat-black interior)"` → `"(U-channel set\nflat-black interior)"`), a tight detail callout went from two lines to three (`"ext. light\nblocked by lip"` → `"ext. light\nblocked by\nlip"`), while a vertical-dimension label with horizontal room to spare was un-wrapped onto one line (`"{DRUM_H}mm\nCLEAR HEIGHT"` → `"{DRUM_H}mm CLEAR HEIGHT"`). Pick the wrap that fits the local gap: more `\n` to squeeze through a narrow vertical channel, fewer to fill a wide-but-short slot. Re-wrapping is often cheaper than moving the label and keeps it anchored where it belongs. (Don't confuse with rule 40/41, which govern the *notes-box* width/spacing, not the line breaks inside an individual `ax.text`/leader label.)

### Leader length — the nearest-pocket search (2026-06-12)

67. **Place a label by sweeping 360° around the leader tip and taking the *nearest* open pocket — the goal is the shortest leader that still lands the text on clear background. Don't default to a habitual direction.** This is the *method* that rules 13/51/56/59 only gesture at. For every callout: hold the tip on the feature, then mentally rotate a full circle — up, down, and every diagonal, not just left/right — and drop the text into the closest gap in *any* of those directions, anchored about one text-line of clearance off the feature edge so the leader stays short. The recurring failure (mine on ibc-frame sheet 1) is reaching the *same way every time* — routing every callout horizontally out to a far margin — when a nearer pocket sat just above, below, or diagonally from the feature, leaving long leaders that strain the eye and make it hard to associate label with part. **Reconciles the earlier rules:** short-leader-into-nearest-gap is the default (rule 13's 75–200mm, rule 56's 3–8 units in details); the long reach into the *outer margin* (rule 59's 200–350+) is the **exception**, used only when the 360° sweep finds no near pocket — i.e. a genuinely packed elevation where the only clear space *is* the margin. When two candidate pockets are similar, prefer the one whose leader crosses the *fewest* filled/hatched bodies (a leader that clears all geometry beats a marginally shorter one that skewers a post). Practical loop: pick the nearest gap, set the anchor ~one line off the edge, render, and only push farther out if the text actually collides.

### Learned from the ibc-frame / ibc-stacking "Tidy labels" pass (2026-06-12)

68. **Keep a notes block narrow by hard-wrapping each item into hanging-indent continuation lines — don't widen the box to fit the longest sentence.** When the only room for the notes is a tight margin, `draw_notes`'s auto-box balloons to whatever the single longest string needs, eating the margin. Instead, pre-wrap each numbered item yourself: split the sentence across several consecutive list entries and indent the continuations with leading spaces so they hang under the *text*, not the number. The user did this across all nine side-elevation notes — e.g. `"1. Three upright bays at 642mm centers. X-braces in bottom-tier"`, `"   bays for racking resistance."` — and shrank the box from `width=1150`→`825` (also `fs 7→6.5`, `spacing 23→20`). A narrow hand-wrapped column tucks into a margin a full-width block can't. (This is rule 66's idea applied to a multi-item notes block, with the added hanging-indent alignment; rules 40/41 still govern the box width/spacing once the wrap is fixed.)

69. **Before settling for a long leader because the near side is "congested," sweep the *opposite* side of the feature and the adjacent open face — prefer a clean face-pocket over a closer-but-cluttered margin.** A correction to how I *apply* rule 67. On ibc-frame sheet 1 I left FLOOR FLANGE FOOT reaching ~389mm into the dimension-packed below-floor band and CAGE TOP RAIL parked out in the side margin, each rationalized as "already the nearest clear pocket." The user's tidy pass moved the floor-flange callout to the open strip just *above* the floor (tip still on the foot, text at Z≈140 in the column) and pulled the cage-rail callout *off* the margin into the open IBC face — both shorter and cleaner. The 360° sweep of rule 67 is only honest if you actually test the far side of the feature and the neighbouring open face, not just the first plausible gap; a margin you "can't avoid" usually has a clean pocket on the other side of the part. Two corollaries from the same pass: give a callout ~one *full* line of clearance from a hatched member, not a token gap (the user nudged D-RING from `POST−30`→`POST−90` off the post); and in these mm elevation sheets the dimension-text `offset=` wants ≈32 with stacked dim rows ≈60 apart — the user bumped my 9–15 → 32/42 across ~10 dims, re-confirming rules 43/55, so stop setting 9–15.
