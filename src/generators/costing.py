...
WATER = [
    ...
    LineItem("Valves and fittings (6× BV ball valves, X1 4-way cross, S60×6 adapters, CV1, SV-01 + SV-02 taps, Blue equalization tie, PVC slip tees/elbows/couplings/transition adapters, 2× service unions)", 1030, 1040, 1049, "2026-07-22: S60×6 adapter re-spec'd female-buttress×2\"NPT + 2→1\" bushing (CPP HMFN/20UD/027) +$48/+$24. 2026-07-27: Banjo valves/tees/elbows/3-ways sourced firm at US Plastic. 1\" PVC slip tees (PVC024001000HD $2.13) + elbows (PVC023001000HD $1.52) + slip×NPT transition adapters (PVC021090600HD $0.79/PVC021091000HD $1.16) firm at Home Depot; filter-jumper bundle retired into itemized pipe/elbows/adapters"),
...
SECTIONS = [
    ...
    Section("5a", "Power & electrical system (solar · 1× LiFePO4 · MPPT · distribution · lighting · protection · master pump switch)", 3243, 3249, 3256),  # KEEP IN SYNC WITH EXPECTED["power"] / total(POWER) — grand_total() sums this Section, not the POWER line items
...
EXPECTED = {                       # the figures the docs are reconciled to (this session)
    ...
    "grand_total": (28768, 33342, 39970),  # 2026-07-28: external power-panel IP enclosure (+$250 net after retiring the flush plate/gasket/frame, +wall gland). Earlier: spray 90° jets, tray shim, transition adapters, IBC strap, X1 cross.
    ...
    "power": (3243, 3249, 3256),   # 2026-07-28: external power panel → McMaster IP enclosure $291.91 (flush plate+gasket+weld-in frame retired, +wall gland). EP backboard SANDEPLY; pull-cord NEMA-4. §5a authoritative subtotal
}
