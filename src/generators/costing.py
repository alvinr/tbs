...
WATER = [
    ...
    LineItem("IBC stacking frame (RHS deep 4-leg box + 4 feet + 8 retaining bars (2/tote face) + 8 identical 2-bolt hangers + 8 backing plates + 8 weld-on lashing rings + 4× 2\" transport straps + 4 anti-slip mats + the 2 walkway center cantilever arms + J6 end/backing plates + crush sleeves + M12×100 + half-lap screws + fab)", 1309, 1590, 1872, "2026-08-19: +$116/$125/$135 — the IBC-owned walkway center arms + J6 connection reconciled here from walkway §6a (they mount on the IBC uprights). 2026-08-18b: +$10/$20 — 16 L-cleat nut backing plates (8 J2 + 8 J7). 2026-08-18: −16 — J2 cleat → L-angle + 1 horizontal bolt. Earlier cost history: RELEASE.md + git log."),
    ...
    LineItem("Captive tee-nut ply-mount hardware (#30 — ¼-20 pronged tee-nuts + zinc machine screws, all ply interfaces)", 46, 46, 46, "2026-08-07 (#30): every REMOVABLE ply-mount joint converts from a wood/lag screw to a zinc machine screw into a back-face 4-prong tee-nut (re-torqueable, no thread-strip). Home Depot: tnut-quarter 825001 $1.57/4pk ×12 + panel-machine-screws ¼-20×1\" 826771 $1.57/4pk ×10 + filter-machine-screws ¼-20×2½\" $1.57 ea ×8 (SKU pending). Interfaces: skid row, ACC-01/02, 5× pump shirt, valve brackets, EP, shelf, Fan-B band, 3× filter housings. 2026-09-07: 5/16 tee-nut retired — filter mount folded into the 1/4-20 ply-mount standard (bracket ear passes 5/16, so 1/4 + washer clamps)."),
...
LIGHTLOCK = [
    ...
    LineItem("SS blind rivets — shell→cap (McMaster 97525A420 multigrip, 100-pack)", 13, 13, 13, "$12.59/100 firm — one multigrip SKU (2026-09-07)"),
    LineItem("SS blind rivets — housing→frame (McMaster 97525A420 multigrip, 100-pack)", 13, 13, 13, "$12.59/100 — same multigrip SKU as shell/rim (2026-09-07)"),
    ...
    LineItem("SS blind rivets — rim-angle→beam (McMaster 97525A420 multigrip, 100-pack)", 13, 13, 13, "2026-08-25: rim-angle flat leg → steel frame beam SS blind-riveted (was TEK, was welded Al→steel), Sheets 9/10. 2026-09-07: → one multigrip SKU 97525A420 $12.59/100 firm; ~24 used."),
...
PANEL = [
    ...
    LineItem("1/8\" 18-8 SS blind rivets — HDPE surround → frame lap (2× 100-pack)", 25, 25, 25, "2026-08-28: surround→frame connection = blind-riveted lap + DP8010 bead. 2026-09-07: → one multigrip SKU 97525A420 $12.59/100 ×2 (shared with the light-trap rivets)"),
