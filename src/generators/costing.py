...
WATER = [
    ...
    LineItem("Filter skid (3× Big Blue 4.5×20 separate housings on mounting brackets + cartridges)", 535, 541, 548, "2026-07-22: U-bracket retired → lag screws to ply backing. 2026-07-25: housing standoff spacers → plywood offcuts. 2026-07-27: filter lag screws firm Everbilt 5-pack $7.09 ×2. 2026-07-27: skid frame → 3× Pentair 150061 zinc single-housing brackets $10.50 (was the $116.76 McMaster angle — purpose-built, −$85)"),
    LineItem("Valves and fittings (6× BV ball valves, X1 4-way cross, S60×6 adapters, CV1, SV-01 + SV-02 taps, Blue equalization tie, PVC slip tees/elbows/couplings/transition adapters, 2× service unions)", 1038, 1048, 1057, "2026-07-22: S60×6 adapter re-spec'd female-buttress×2\"NPT + 2→1\" bushing (CPP HMFN/20UD/027) +$48/+$24. 2026-07-27: Banjo valves/tees/elbows/3-ways sourced firm at US Plastic. 1\" PVC slip tees (PVC024001000HD $2.13) + elbows (PVC023001000HD $1.52) + slip×NPT transition adapters (PVC021090600HD $0.79/PVC021091000HD $1.16) firm at Home Depot; filter-jumper bundle retired into itemized pipe/elbows/adapters. 2026-07-28: transition-adapter P&ID takeoff — ½\" 18→22, 1\" 22→26 (+$8) from the plumbing joint-convention trace"),
    ...
    LineItem("Processing tray (304 SS panels + fabrication, shim strips, sump pickup, liner, hardware)", 1538, 1883, 2226, "2026-07-23: slope shims firmed to the 1-1/4\" HDPE plate route ($210–300, from the $40–75 flat-bar est). 2026-07-27: tray liner folded into ldpe-sheeting roll (−$8) + Loctite PL Premium 2-pack $11.94 (−$3)"),
    LineItem("Spray bar assembly (40×25 SS RHS beam, side LDPE manifold, 40 jets, manifold + 7 feed tubes, 4 Ø32 wheels, ball joint, arm + turned adapter + jam nut + clamp collar, hose)", 385, 421, 457, "2026-07-22: arm jam nut/collar/self-tap/beam-clamp re-priced to real SKUs. 2026-07-25: arm tube → McMaster 9056K36 8ft stock $64.03 (was a $6 500mm cut, too short to order) +$58"),
...
SECTIONS = [
    ...
    Section("5a", "Power & electrical system (solar · 1× LiFePO4 · MPPT · distribution · lighting · protection · master pump switch)", 3009, 3037, 3064),  # KEEP IN SYNC WITH EXPECTED["power"] / total(POWER) — grand_total() sums this Section, not the POWER line items
...
EXPECTED = {                       # the figures the docs are reconciled to (this session)
    ...
    "grand_total": (28542, 33138, 39786),  # 2026-07-28: +$8 plumbing transition-adapter P&ID takeoff (½" 18→22, 1" 22→26). Earlier: external power panel → fabricated penetration box + flashing (McMaster IP enclosure retired); MC4 bulkheads, cooler outlet, PV disconnect, spray 90° jets.
    "walkway": (1979, 2395, 2825),  # 2026-07-23b: bump extended 2nd rib toward IBC — 1 near bracket std->widened (+$12/$22)
    "water": (6633, 7286, 7938),  # 2026-07-28: spray nozzles → 90° down-jets, pitch 150→100mm (26→39, 4× DIG 110B 10-packs); carbon Aquaboon; SV taps 36903; nitrile firm; tray shims $295.96; transition-adapter P&ID takeoff +$8 (½" 18→22, 1" 22→26).
