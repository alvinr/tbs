...
def service_loads():
    ...
    # arm->upright connection = a BOLTED END-PLATE (plate welded to the arm end, bolted through the upright to
    # a rear backing plate, per ibc_cantilever_arms). BEARING-TYPE: BOTH M12 sit ABOVE the arm (a tension bolt
    # group); the plate bears compression at the arm's lower flange. Lever = bolt-group centroid (~Z155) to the
    # compression at the arm bottom (~Z90) ≈ 54mm; the 2 bolts SHARE the tension. Both moved above the arm to
    # clear the corridor bottom frame rail (Z12-63) AND the welded arm — down-load only, no uplift (2026-08-17).
    lever = 0.065
    f_bolt = m_conn / lever / 2.0      # total tension over the lever, shared by the 2 tension bolts
    ...
    lines.append(f"  arm->upright END-PLATE (2x M12 ABOVE arm, bearing-type, {lever*1000:.0f}mm lever): {f_bolt:.0f} N/bolt tension,"
...
def render_png(path=None):
    ...
    sf_endplate = endplate_cap / (m_conn / 0.065 / 2.0)   # bearing-type: 2 M12 ABOVE the arm share the tension over a ~54mm lever to the compression at the arm bottom
