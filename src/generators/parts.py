...
PARTS: list[Part] = [
    ...
    Part("axial-fan-150", "150×150×50mm axial fans", "ducting-ventilation",
         "ventilation", 2, "ea", 25, 25, "Digi-Key", spec="12V DC, ~150–200 CFM each (GDSTIME/Wathai 15050)",
    ...
    Part("corridor-panel-ply-18", "Corridor plumbing-panel ply (23/32\" exterior)", "timber-ply",
         ...
         spec='4×8 ft 23/32\" (18mm) RTD Southern Yellow Pine exterior sheathing — rear backing board + drain-riser spine + spacer offcuts. STANDARD exterior per project rule (was mis-spec\'d marine ~$179-250). Firm $29.30 (Home Depot 2026-07-23). Seal cut edges.',
         panel="Corridor"),
    ...
    Part("valve-v050fp-corridor", 'Banjo V050FP ball valve 1/2" FNPT', "plumbing-fittings",
         "water", 3, "ea", 44.27, 44.27, "US Plastic Corp", "Amazon", spec="PP full-port quarter-turn; pump-suction isolation BV-01 (P-01), BV-02 (P-05), BV-06 (P-03)", panel="Corridor", part_no="30651", url="https://www.usplastic.com/catalog/item.aspx?itemid=30651"),
    ...
    Part("valve-3way-half", '3-way diverter valve 1/2" FNPT', "plumbing-fittings",
         "water", 1, "ea", 23.99, 23.99, "US Plastic Corp", spec="L/T-port PVC-compatible; 3W-DV-02 (tray drain)", panel="Corridor", part_no="22365", url="https://www.usplastic.com/catalog/item.aspx?itemid=22365"),
    ...
    Part("water-powerpole", "Anderson Powerpole connectors 30A", "electrical-distribution",
         "water", 5, "pair", 2, 2, "Powerwerx", spec="Pump connections — one pair per pump (P-01..P-05)"),
    ...
    Part("ibcf-hangers", "4mm folded plate", "steel-structural",
         "ibc-frame", 4, "ea", 7.5, 12.5, "local fab", spec="Simpson-style U-pocket wall joist hangers (4 off) — each through-bolted (4× M12×65) to an exterior 100×135×8 backing plate on the outside of the container side wall."),
    ...
    Part("ibcf-strap", '2" (50mm) ratchet strap, 3,333 lb WLL', "fasteners-hardware",
         "ibc-frame", 4, "ea", 16.98, 16.98, "Home Depot", part_no="82827", url="https://www.homedepot.com/p/331257450", spec="Transport securing, over each stack. Keeper 82827 heavy-duty 2\"×27ft, 3,333 lb (~1,512 kg) WLL / 10,000 lb break — width corrected 25mm→50mm (a 1\" strap can't hold the 1,100 kg the restraint needs)."),
    ...
    Part("bolt-m12x40", "M12×40 hex bolt, Grade 8.8", "fasteners-hardware",
         "ibc-frame", 8, "ea", 21.91 / 25, 21.91 / 25, "McMaster-Carr", part_no="91280A720",
         url="https://www.mcmaster.com/91280A720/",
         spec="Front-bar → upright angle cleats (2 each × 4 bars = 8). Short grip (bar + cleat + upright flange) → M12×40 fully threaded is correct. McMaster 91280A720: M12×1.75×40, Class 8.8 zinc, fully threaded — pitch + length confirmed vs the 91280A720 PDF 2026-07-29 (matches nut-m12-plain). $21.91/pack of 25 (8 used). (Wall-hanger through-bolts are the M12×65 below.)"),
    Part("bolt-m12x65", "M12×65 hex through-bolt, Grade 8.8 zinc, partial-thread", "fasteners-hardware",
         ...
         spec="IBC wall-hanger through-bolts (4 each × 4 hangers = 16) — through the corrugated side wall to the exterior 100×135×8 backing plate (hex heads outside). Grip = 8mm plate + ~30mm corrugation + 4mm hanger flange ≈ 42–54mm → M12×65 partial-thread (the fully-threaded M12×40 could not span it). $15.95/pack of 10 → 2 packs for 16. Pad with 1–2 M12 flat washers if the actual corrugation is <30mm."),
    ...
    Part("ibcf-wall-backing", "Steel backing plate 100×135×8mm", "steel-structural",
         "ibc-frame", 4, "ea", 6, 10, "Metal Supermarkets", spec="Exterior wall backing plates (one per hanger) — flat 100×135×8mm steel on the OUTSIDE of the container side wall (hex heads outside), 4× M12 holes; spreads the totes' transport thrust into the thin corrugated wall so the through-bolts can't pull through."),
    ...
    Part("spray-retainer-clips", 'SS/nylon retainer clips for 3/4" LDPE', "fasteners-hardware",
         "spray", 2, "ea", 2, 2, "DripDepot", spec="Fold-back end closures"),
    ...
    Part("spray-brass-barb", '1/2" PVC barbed tee (flex hose → manifold center feed)', "plumbing-fittings",
         "spray", 1, "5-pack", 2.85, 2.85, "DripDepot", part_no="1084", url="https://www.dripdepot.com/barb-tubing-tee-size-half-inch", spec="DripDepot 1084 ½\" PVC barbed tee (Alvin 2026-07-28: PVC, NOT brass as previously specced), $0.57 ea × 5-pack = $2.85; the SINGLE center-feed inlet — ½\" flex hose → manifold center (Option 1: single feed replaced the 7-tube distribution fan). 1 used. Firm 2026-07-28."),
    ...
    Part("spray-braided-hose", '1/2" reinforced braided PVC hose, 15 ft', "plumbing-fittings",
         "spray", 1, "ea", 15, 15, "DripDepot", spec="BV-02 to beam feed (4 m coiled)"),
    ...
    Part("mppt-charge-fuse", "MPPT charge-line fuse — 60A ANL + holder", "electrical-distribution",
         "electrical", 1, "ea", 37, 42, "Amazon", "Waytek Wire", part_no="5005", url="https://www.amazon.com/dp/B000THT1SG",
         spec="Blue Sea 5005 ANL fuse block — Amazon B000THT1SG $29.53 (cheaper than Waytek 5005 $33.97) + Blue Sea 5123 60A ANL fuse ~$8 (est — the fuse price still to confirm). Protects the MPPT→battery 6 AWG charge lead close to the battery (§7.3). Holder firm (Alvin 2026-07-30)."),
    ...
    Part("wire-14awg-red", "14 AWG tinned hook-up wire, red — 100ft (Circuit C feed / AC)", "electrical-distribution",
         ...
         spec="Circuit C pump feed + inverter AC out + Circuit G white-LED feed (~14m; G moved up from 16 AWG for the ~7.4A COB load). Tinned copper UL-1015. WRT14-2 (red, confirmed); price est — confirm at order."),
    ...
    Part("anderson-powerpole", "Anderson Powerpole 30A connectors, 50 pairs (unassembled)", "electrical-distribution",
         "electrical", 1, "kit", 53.99, 53.99, "Powerwerx", url="https://powerwerx.com/anderson-powerpole-connectors-30amp-unassembled",
         spec="30A unassembled Powerpole set, 50 pairs (PP15/45 housings + 30A contacts). $53.99 firm (Alvin 2026-07-30, up from the $40 est). Powerwerx identifies it by the product page; no separate catalog SKU."),
    ...
    Part("led-strip", "HitLights 12V COB LED strip 4000K, 16.4ft reel (Circuit G, ×2)", "electrical-distribution",
         ...
         spec="12V DC COB LED strip, 4000K neutral, 16.4ft reel — 426 lm/ft, 4.2 W/ft, CRI 90+, dimmable, cuttable. Circuit G white work lighting = 3 ceiling runs, each ~2,162mm: 2 parallel to the X=2270/4170 red safelights + 1 over the plumbing panel toward the cargo door (~6.5m total → ~9,000 lm ≈ ~650 lux, ~89W/7.4A). 2 reels cover the 3 runs (21.3 ft). Circuit G fed in 14 AWG (up from 16) for the ~7.4A load. PRIMARY: L2712V-40D3-1630-U premium UL-listed white PCB $42.28; cheaper alt L0512V-403-1630-U-BLK standard black PCB $37.28 (both 4000K, confirmed Alvin 2026-07-30). Mounts in led-channel; COB = even, hot-spot-free; dimmable."),
    Part("led-channel", "LED Profiles 981 slimline channel + diffuser, 8 ft (×3)", "electrical-distribution",
         "electrical", 3, "8ft length", 27.00, 27.00, "LED Profiles", part_no="981ASL", url="https://ledprofiles.com/collections/all-led-channels/products/slimline-ultra-low-profile-led-channel-981-series",
         spec="Alberko/LED Profiles 981ASL Ultra-Low-Profile aluminum LED channel (6063-T5 clear-anodized, 14.9×6.86mm) with snap-in FROSTED diffuser (D34) — houses the Circuit-G 12V COB strip, even hot-spot-free wash. Fits strips up to 12.70mm (spec-sheet confirmed 2026-07-30), so the ~8mm HitLights strip seats fine. 3× 8 ft (2.44m) sticks — one per white run (each ~2.16m; 8 ft covers the run with trim). $27.00 ea ($25.65 @5+). End caps + mounting clips are accessories — confirm bundled or add (~$5). The red safelight strips mount direct (no channel)."),
    Part("led-connectors", "LED strip connectors + 12V PWM dimmers (Circuits G + D)", "electrical-distribution",
         "electrical", 1, "lot", 15, 30, "Amazon",
         spec="Solderless clip connectors (strips → 12V circuit wire) + inline 12V PWM dimmers — one for the Circuit-G white run, one for the Circuit-D red safelight (both dimmable). Est $15–30 — confirm at order."),
    Part("safelight-strip", "12V red LED safelight strip (Circuit D, 3× 2.16m)", "electrical-distribution",
         "electrical", 1, "lot", 25, 45, "Amazon", "HitLights",
         spec="Red 12V LED strip for the Circuit-D safelight — 3 ceiling runs (X=520/2270/4170, each 2,162mm, near→far wall, shortened to clear the optical cone) matching overview.skp, ~15W total. Cyanotype responds only to UV/blue, so a red safelight is safe. Keep 'as-is' (Alvin): LOW-power red (SMD ~2-3 W/m to hold the ~15W spec — NOT a COB, which would be ~90W). ~7m of red 12V strip. Direct-mount (no channel). SKU/price PENDING — source a red 12V strip; est $25–45. (Modeled + described in the report but was never in the BOM — this closes that gap.)"),
    ...
    Part("panel-corner-stiffener", '1"×1"×1/8" Al angle, 8 ft — corner-zone stiffener grid', "aluminum",
         ...
         spec="Corner-zone anti-oil-can rib grid (replaces the former solid 3mm 5052 Al core — SS/weight audit 2026-07-29, which found the plate redundant: light-tightness is the two black HDPE skins, latch/fan load is the RHS frame + ply band, so the plate only stiffened). Per corner: 1 vertical (2,258mm) + 2 horizontal (653mm) 1\"×1\"×1/8\" (25×25×3.2mm) Al angle ribs, ~325×750mm bays, holding both 1/8\" HDPE skins flat within the 40mm framed cavity. The leaf is VERTICAL, so skin self-weight is in-plane; the grid only resists out-of-plane oil-can (works with the U-channel skin retainers at ~400-450mm centers, report §2.5). ~7.1m installed → 4× 8 ft (2,438mm) sticks for clean piece-fit (2 sticks → the 2 verticals, 2 → the 4 horizontals + spare). ~2.9 kg vs the plate's ~25 kg. Grainger 2EYP1 $12.20/8ft firm (Alvin 2026-07-29); the 12 ft 897KL0 was $88.80 = ~5× the per-ft price, rejected."),
