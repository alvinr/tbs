...
PARTS: list[Part] = [
    ...
    Part("shade-cloth-80", "Shade cloth — 70% (10×20 ft)", "fabric-textile",
         "ventilation", 1, "ea", 40, 40, "Amazon", "Farm supply", part_no="B075J93DTJ", url="https://www.amazon.com/dp/B075J93DTJ",
         spec="10×20 ft shade cloth with grommets. 80% grade is uncommon on Amazon → 70% (B075J93DTJ) or 90% (B07S7Z24FL) dominate; picked 70% (price corrected $80→$40, the old est was high). Any 70–90% works for the cooler/canopy shade."),
    ...
    Part("coiled-cable-16awg", "16 AWG coiled (retractile) cable, 2-cond", "electrical-distribution",
         "ventilation", 1, "ea", 15, 15, "Amazon", "Waytek Wire", part_no="B00BSORUSW", url="https://www.amazon.com/dp/B00BSORUSW",
         spec="1m, 2-conductor retractile — Fan B flex. Unique Dynamics 16AWG 2-cond coiled cord (B00BSORUSW), stretches ~4ft. Jacket is PVC (silicone-coiled 16AWG is rare/custom) — PVC is fine for this dry interior flex. Waytek has no matching coiled cable. Price est."),
    ...
    Part("spray-retainer-clips", 'SS/nylon retainer clips for 3/4" LDPE', "fasteners-hardware",
         "spray", 2, "ea", 2, 2, "Amazon", "DripDepot", part_no="B0CY5R6VTL", url="https://www.amazon.com/dp/B0CY5R6VTL",
         spec='Figure-8 fold-back end closures for 3/4" poly tubing — Amazon 10-pack (B0CY5R6VTL). DripDepot figure-8 3/4" is a bulk pack (~$49); the 10-pack suits the few needed. Price est.'),
    ...
    Part("spray-braided-hose", '1/2" reinforced braided PVC hose, ~15 ft', "plumbing-fittings",
         "spray", 1, "ea", 15, 15, "Home Depot", part_no="T12006003", url="https://www.homedepot.com/p/304185193",
         spec='BV-02 → beam feed (~4 m coiled). UDP 1/2"ID×3/4"OD clear braided vinyl (T12006003), sold in 10 ft — order 2 (or a 25/50 ft HYDROMAXX coil) for the ~15 ft run. DripDepot does not stock braided PVC. Price est.'),
    ...
    Part("spray-ball-joint", "Ø20mm ball joint, zinc socket, M12 stud", "bearings-motion",
         "spray", 1, "ea", 12, 12, "Amazon", part_no="B076XM3VXC", url="https://www.amazon.com/dp/B076XM3VXC",
         spec="Multi-axis arm articulation. Representative: AlveyTech M12×1.25 tie-rod ball joint (B076XM3VXC) — many go-kart equivalents. CONFIRM the ~Ø20mm ball + M12 stud + socket material at order (this one is steel, M12×1.25)."),
    ...
    Part("wire-14awg-red", "14 AWG tinned hook-up wire, red — 100ft (Circuit C feed / AC)", "electrical-distribution",
         ...
         spec="Circuit C pump feed + inverter AC out + Circuit G white-LED feed (~14m; G moved up from 16 AWG for the ~6.3A COB load). Tinned copper UL-1015. WRT14-2 (red, confirmed); price est — confirm at order."),
