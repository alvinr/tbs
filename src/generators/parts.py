...
PARTS: list[Part] = [
    ...
    Part("ll-wiper-brush", "Nylon strip brush — running-gap light-seal wiper (×4 lines)", "seals-gaskets",
         "lightlock", 4, "7ft", 27.02, 27.02, "McMaster-Carr", part_no="74715T2",
         url="https://www.mcmaster.com/74715T2-74715T207/",
         spec="4 vertical strips RIVETED to the rotating drum OD — the running-gap light seal (drum↔housing). 1/8\"×1/8\" stainless-backed, 0.008\" black nylon, 1\" overall / 7/8\" trim; one 7 ft piece per line (7ft=2134mm ≥ 2120mm drum ht) → 4× 7ft. $27.02/7ft firm.", note="confirmed 2026-08-24 (SKU+URL+price)"),
    ...
    Part("ll-grab-rail", "100mm Ø SS grab rail", "fasteners-hardware",
         "lightlock", 1, "ea", 25, 45, "Marine Fiberglass Direct", spec="Interior handle, 400mm cut length — welded to the handle stile (ll-handle-stile), not the drum wall", url="https://www.marinefiberglassdirect.com/products/16-stainless-steel-safety-grab-bar-bolt-on-for-marine-dock-deck-boat-pool-hot-tub"),
    Part("ll-handle-stile", "40×40 SS RHS grab-rail stile (cap→cap) + M12 cap bolts", "steel-structural",
         "lightlock", 1, "lot", 45, 85, "Metal Supermarkets", "Online Metals",
         spec="Steel stile (40×40×3 SS RHS, ~2.1 m) spanning + bolted between the two Al caps (4× M12 through-bolt + nut) — carries the grab-rail pull load into the structural caps, NOT the thin HDPE wall. Grab rail welds to a lug on the stile. Est.; firm at order.", note="PRICE-VERIFY (Aug-2026 re-price)"),
    ...
    Part("ll-fasteners", "Stainless bolts (cap/ring/edge/stile) + nylon isolation washers", "fasteners-hardware",
         "lightlock", 1, "lot", 45, 70, "US Plastic + McMaster", spec="Cap→flange 4×M10 ×2 (into the tapped steel flange), ring/collar→beam 6+8 M10 (captive/tapped nuts on the 3mm RHS beam wall — mechanism per Sheets 5/10) + nylon isolation shoulder washers at every steel↔Al/HDPE joint (no galvanic couple). Rivets, edge-channel M8, and stile M12 are separate lines.", part_no="92674", url="https://www.usplastic.com/catalog/item.aspx?itemid=155501"),
    Part("ll-fabrication", "Fabrication — roll + weld 2 HDPE cylinders, roll rim-angle, fit metal caps/bearings", "fabrication-labor",
         "lightlock", 1, "lot", 800, 1150, "Local plastic + metal fab", spec="~16–22 hrs: roll + hot-air/extrusion-weld the 2 HDPE cylinders, roll the Al rim-angle rings, fit the metal caps + SKF 6215 bearings + stub shafts, rivet the lap joints, mount the drum brush + edge channels + grab-rail stile."),
