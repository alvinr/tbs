...
PARTS: list[Part] = [
    ...
    Part("filter-jumper", '1" PVC inter-housing jumpers', "plumbing-fittings",
         "water", 1, "lot", 18, 32, "Ferguson", spec='F-01 OUT→F-02 IN, F-02 OUT→F-03 IN — 1" PVC + 90° elbows routed outside the bodies (THREADED — filter ports are hard components)', panel="Pinhole Wall"),
    ...
    Part("sample-tap-sv01", 'pH sample tap (SV-01) — 1/2" PP ball valve + barb spout + branch tee', "plumbing-fittings",
         "water", 1, "ea", 10, 18, "US Plastic Corp", spec='Filtered-water sample draw before 3W-DV-01; Banjo V050FP 1/2" PP ball valve + downturned 1/2" hose barb on a 1"×1/2" reducing branch tee, panel face above spill line', panel="Pinhole Wall"),
    Part("sample-tap-sv02", 'pH sample tap (SV-02) — 1/2" PP ball valve + barb spout + branch tee', "plumbing-fittings",
         "water", 1, "ea", 10, 18, "US Plastic Corp", spec="pH sample on the P-04 tray-drain discharge, before 3W-DV-02; same build as SV-01", panel="Corridor"),
    ...
    Part("elbow-half", '1/2" NPT 90° elbow polypropylene', "plumbing-fittings",
         "water", 14, "ea", 0.74, 0.74, "Home Depot", part_no="PVC023000600HD", url="https://www.homedepot.com/p/203812033", spec="All pump-driven run bends. 2026-07-27: Charlotte PVC Sch40 90° S×S (SLIP) — desc says NPT; confirm slip-vs-threaded in the plumbing audit"),
    Part("elbow-el100", '1" PVC Sch-40 slip 90° elbow', "plumbing-fittings",
         "water", 4, "ea", 0.70, 1.30, "Home Depot", spec='1" PVC slip run bends (joint convention §5.1): IBC bends, filter outlet to DV-01. 2026-07-27 fork b — was threaded Banjo FRPP $4.59. Source exact Home Depot SKU/price (~$1).'),
    ...
    Part("tee-100", '1" PVC Sch-40 slip tee', "plumbing-fittings",
         "water", 4, "ea", 1.20, 2.00, "Home Depot", spec='1" PVC slip run tees (joint convention §5.1): 3× IBC drain + 1× X1 fill split to both Blue totes. 2026-07-27 fork b — was threaded Banjo FRPP $14.19; consolidated tee-100 + tee-100-hdpe (×4). Source exact Home Depot SKU/price (~$1.50).'),
    Part("pvc-transition-adapters", "PVC slip×MNPT male adapters (run → threaded components)", "plumbing-fittings",
         "water", 40, "ea", 0.5, 1.0, "Home Depot", spec='Joint convention §5.1 (fork c): one slip×NPT male adapter where the glued PVC run lands on each threaded component — ball/diverter/check valves, filter housing ports, tank bulkheads, sample taps, accumulator. ½" + 1" mix, ~40 est; exact count/size split from the schematic trace. Source exact Home Depot SKUs.'),
    ...
    Part("blue-equalization-tie", '1" bulkhead tank-body fittings (Blue equalization cross-tie)', "plumbing-fittings",
         "water", 2, "ea", 12.62, 12.62, "US Plastic Corp", spec='Low tank-body penetration in each Blue tote (IBC-1 + IBC-2) for the 1" equalization cross-tie that self-balances the two Blue levels (run made from the 1" PVC stock). Cost est.', part_no="32194", url="https://www.usplastic.com/catalog/item.aspx?itemid=32194"),
    ...
    Part("pvc-1in", '1" PVC Sch-40 pipe', "plumbing-fittings",
         "water", 1, "stick", 12, 18, "Ferguson", spec="PVC Sch-40 solvent-weld, ~20 ft; filter outlet + IBC lines. Re-price/re-count when sourced (10-ft sticks)"),
    Part("pvc-three-quarter", '3/4" PVC Sch-40 pipe', "plumbing-fittings",
         "water", 2, "stick", 10, 15, "Ferguson", spec="Spray bar run, PVC Sch-40 solvent-weld. Re-price/re-count when sourced (10-ft sticks)"),
    ...
    Part("bolt-m12x40", "M12×40 hex bolt, Grade 8.8", "fasteners-hardware",
         "ibc-frame", 8, "ea", 0.71, 1.44, "FMW Fasteners", "US Bolt Kits", part_no="1634027",
         url="https://www.fmwfasteners.com/products/m12-1-75-x-40-hex-cap-screw-8-8-din-933-zinc-plated-fully-threaded",
         spec="Front-bar → upright angle cleats (2 each × 4 bars = 8). Short grip (bar + cleat + upright flange) → M12×40 fully threaded is correct. M12×40 8.8 zinc DIN933; FMW 1634027 $1.44ea / US Bolt Kits $0.71ea. (Wall-hanger through-bolts are the M12×65 below.)"),
    ...
    Part("nut-m12-plain", "M12 hex nut, plain", "fasteners-hardware",
         "ibc-frame", 16, "ea", 12.78 / 50, 12.78 / 50, "McMaster-Carr", part_no="90591A181", url="https://www.mcmaster.com/90591A181/", spec="Plain hex nut (inside the container) — M12×65 wall-hanger through-bolts (+ split lock washer). $12.78/pack of 50."),
    ...
    Part("tray-suction-hose", '1" reinforced suction hose, 6 ft', "plumbing-fittings",
         "tray", 1, "ea", 15, 15, "US Plastic Corp", spec="Pickup tube to P-04"),
    ...
    Part("spray-nozzles", "Flat-fan irrigation spray nozzles, barbed", "plumbing-fittings",
         "spray", 26, "ea", 30 / 26, 50 / 26, "DripDepot", spec="180° fan pattern; side-tapped into the poly manifold, spray down-and-in"),
    ...
    Part("spray-feed-tube", '1/4" irrigation poly tube', "plumbing-fittings",
         "spray", 1, "ea", 6, 6, "DripDepot", spec="Manifold to beam feed points (~7m total)"),
    Part("spray-barbed-feed", "Barbed tees, tube into the side poly manifold", "plumbing-fittings",
         "spray", 7, "ea", 10 / 7, 10 / 7, "DripDepot", spec="Feed tube to the side poly manifold, 7 feed points"),
    ...
    Part("spray-brass-barb", '1/2" barb × 1/2" hose barb, brass', "plumbing-fittings",
         "spray", 1, "ea", 4, 4, "DripDepot", spec="Flex hose to manifold inlet"),
    ...
    Part("fp-cross-slide", "316 flat-bar Z (tilt) + X (swing) cross-slides + UHMW pad + gib", "steel-structural",
         ...
         spec="One 2-axis cross-slide stack per corner — 316 flat-bar Z (tilt) + X (swing) slides on UHMW pads with an adjustable gib. FLAT-BAR STOCK: 316 SS ¼\"×1½\" (6.35×38.1mm); 8 pieces (4× ~250mm Z-tilt + 4× ~260mm X-swing) ≈ 2.05m cut length → order ~2.4m (one 8ft length) to allow kerf + mounting overlap. UHMW pad $23–93/sheet + brass-tip gib separate. Firm at order (est.)."),
    ...
    Part("shelf-pvc-pipe", '½" PVC Sch-40 pipe (tap relocation)', "plumbing-fittings",
         "shelf", 1, "lot", 10, 10, "Irrigation supply", spec="extend the blue supply trunk ~1.3 m left to TAP-01 (PVC Sch-40, per the joint convention)"),
    ...
    Part("nut-m12-plain", "M12 hex nut, plain", "fasteners-hardware",
         "walkway", 59, "ea", 12.78 / 50, 12.78 / 50, "McMaster-Carr", part_no="90591A181", url="https://www.mcmaster.com/90591A181/", spec="Plain hex nut — M12×65 cantilever bolts (+ split lock washer). $12.78/pack of 50."),
    Part("washer-m12-flat", "M12 flat washer, zinc", "fasteners-hardware",
