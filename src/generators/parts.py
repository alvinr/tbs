...
PARTS: list[Part] = [
    ...
    Part("wire-14awg-red", "14 AWG tinned hook-up wire, red — 100ft (Circuit C feed / AC)", "electrical-distribution",
         ...
         spec="Circuit C pump feed + inverter AC out (~9m). Tinned copper UL-1015. WRT14-2 (red, confirmed); price est — confirm at order."),
    Part("wire-14awg-blk", "14 AWG tinned hook-up wire, black — 100ft (Circuit C feed / AC)", "electrical-distribution",
         ...
         spec="Circuit C return (~9m). Tinned copper UL-1015. WRT14-0 (black, pattern-inferred); confirm SKU/price at order."),
    Part("wire-16awg-red", "16 AWG tinned hook-up wire, red — 100ft (Circuits A/B/G + branches)", "electrical-distribution",
         ...
         spec="Circuits A/B fans, G LED, C pump branches — the biggest run (~25m). Tinned copper UL-1015. WRT16-2 (red, confirmed); price est — confirm at order."),
    Part("wire-16awg-blk", "16 AWG tinned hook-up wire, black — 100ft (Circuits A/B/G + branches)", "electrical-distribution",
         ...
         spec="Circuits A/B/G/C-branch returns (~25m). Tinned copper UL-1015. WRT16-0 (black, confirmed); price est — confirm at order."),
    ...
    Part("led-strip", "HitLights 12V COB LED strip 4000K, 16.4ft reel (Circuit G)", "electrical-distribution",
         "electrical", 1, "reel", 37.28, 42.28, "HitLights", part_no="L2712V-40D3-1630-U",
         url="https://hitlights.com/products/premium-12v-cob-led-strip-light-single-color-ul-listed-16-4ft-ip-20-white-pcb",
         spec="12V DC COB LED strip, 4000K neutral, 16.4ft reel — 426 lm/ft, 4.2 W/ft, CRI 90+, dimmable, cuttable. Circuit G general lighting: run ~10ft (~4,260 lm → ~300 lux over 14 m²) at ~42W/3.5A — fits 16 AWG/10A, no rewire (spare strip on the reel). PRIMARY: L2712V-40D3-1630-U premium UL-listed white PCB $42.28; cheaper alt L0512V-403-1630-U-BLK standard black PCB $37.28 (both 4000K, confirmed Alvin 2026-07-30). Mounts in led-channel; COB = even, hot-spot-free."),
    Part("led-channel", "LED Profiles 981 slimline channel + diffuser (3× 1m)", "electrical-distribution",
         "electrical", 3, "length", 27.00, 27.00, "LED Profiles", part_no="981", url="https://ledprofiles.com/collections/all-led-channels/products/slimline-ultra-low-profile-led-channel-981-series",
         spec="LED Profiles 981 Slimline Ultra-Low-Profile aluminum LED channel with opal diffuser — houses the Circuit-G 12V COB strip (surface-mount, gives the even hot-spot-free wash). $27.00 ea, $25.65 at 5+. 3× 1m = ~10 ft to match the ~10 ft lit run (WHITE_LIGHT_W=42W). CONFIRM off the spec sheet (Spec_Sheet_981_Series_NEW_A.pdf): (1) STICK LENGTH — assumed 1m (the 5-unit price break = 5m = the full HitLights reel); if 2m, only 2 sticks (~$54) are needed. (2) MAX STRIP WIDTH ≥ the HitLights strip (~8mm). For full wall-to-wall even coverage run the whole reel: 5× 1m = $128.25 @ $25.65 (then dim, or bump WHITE_LIGHT_W)."),
    Part("led-connectors", "LED strip connectors + 12V PWM dimmer (Circuit G)", "electrical-distribution",
         "electrical", 1, "lot", 10, 20, "Amazon",
         spec="Solderless clip connectors (strip → Circuit-G 12V wire) + an inline 12V PWM dimmer (the COB strip is dimmable). Est $10–20 — confirm at order."),
