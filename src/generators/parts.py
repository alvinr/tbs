...
#   • 316 — used ONLY by deliberate choice, not necessity: `bolt-m6-tray` (standing-wash tray seam, a
...
#     it here (2026-08-13). Small M6/M8 wet-zone fasteners went zinc→304 (modest load, A2-70 fine).
...
PARTS: list[Part] = [
    ...
    # shurflo-bracket RETIRED 2026-07-31 — the mounting bracket ships with the Shurflo 2088-554-144 pump (confirmed); −$50.
    ...
    Part("filter-skid-frame", "Big Blue housing mounting brackets (×3)", "water-equipment",
         "water", 3, "ea", 10.50, 10.50, "Fresh Water Systems", part_no="244718", url="https://www.freshwatersystems.com/products/mounting-bracket-white-single-housing-for-10-20-big-blue-housings", spec="Pentair 244718 zinc single-housing mounting bracket (BRACKET ONLY), one per 4.5×20 Big Blue (×3), machine-screwed to the 18mm ply backing via back-face pronged tee-nuts (#30, re-torqueable). Purpose-built — replaces the welded slotted-angle frame (2026-07-27). Switched 2026-09-07 from the 150061 KIT to the bracket-only 244718 — we supply our own 1/4-20 machine screws + tee-nuts, so the kit's hardware was redundant. Firm $10.50 (Fresh Water Systems 2026-09-07).", panel="Pinhole Wall"),
    ...
    Part("filter-machine-screws", "Machine screws — filter housings to ply tee-nuts (1/4-20)", "fasteners-hardware",
         "water", 8, "ea", 9.93 / 100, 9.93 / 100, "McMaster-Carr", part_no="90272A552", url="https://www.mcmaster.com/90272A552/", spec="2 per housing × 3 = 6 needed (+2 spare) — 1/4\"-20 × 2½\" machine screw + a 1/4 flat washer under each head (filter-washer), through the bracket ear + 25mm standoff + 18mm ply into a back-face pronged tee-nut (tnut-quarter). Downsized from 5/16\"-18 (2026-09-07) to fold the filter mount into the 1/4-20 ply-mount standard — the bracket ear already passes 5/16, so 1/4 + washer clamps with the washer spanning the ear bore; retires the 5/16-18 family. Re-torqueable, serviceable. Dry backboard mount behind the housings (not immersion). McMaster 90272A552 $9.93/pack of 100 firm (2026-09-07).", panel="Pinhole Wall"),
    Part("filter-washer", "1/4\" flat washer — filter-housing machine screws", "fasteners-hardware",
         "water", 8, "ea", 7.47 / 50, 7.47 / 50, "McMaster-Carr", part_no="90850A100", url="https://www.mcmaster.com/90850A100/", spec="1/4 flat washer under each filter-machine-screw head (8 = 2/housing × 3 + 2 spare) — spans the Big Blue bracket ear bore (sized for 5/16) so the downsized 1/4 screw clamps without slop. McMaster 90850A100 $7.47/pack of 50 firm (2026-09-07).", panel="Pinhole Wall"),
    Part("tnut-quarter", "1/4\"-20 pronged tee-nut (ply-mount interfaces)", "fasteners-hardware",
         "water", 12, "4-pack", 1.57, 1.57, "Home Depot", part_no="825001", url="https://www.homedepot.com/p/Everbilt-1-4-in-20-Zinc-Plated-Tee-Nut-4-Pack-825001/317478995", spec="Everbilt 1/4\"-20 zinc 4-prong tee-nut, 5/16\" (8mm) barrel — seats from the BACK of the 18mm ply. Captive re-torqueable anchor for every ¼-20 ply-mount interface (#30, 2026-08-07): P-04/SV-02/DV-02 skid row (6), ACC-01/02 (4), pump-mount shirt 5× Shurflo (10), valve brackets (6), EP panel (4), chem shelf (4), Fan-B band (2), 3× filter housings (6, absorbed from the retired 5/16 tee-nut 2026-09-07) = 42 + 6 spare → 12× 4-pack. Mostly water; the EP/shelf/Fan-B pieces (~10) are minor cross-system, carried on this one hardware line. Firm $1.57/4-pack (Home Depot 2026-08-07)."),
    Part("panel-machine-screws", "1/4\"-20 zinc machine screws (ply-mount interfaces)", "fasteners-hardware",
         "water", 10, "4-pack", 1.57, 1.57, "Home Depot", part_no="826771", url="https://www.homedepot.com/p/Everbilt-1-4-in-20-x-1-in-Combo-Truss-Head-Zinc-Plated-Machine-Screw-4-Pack-826771/317479749", spec="Everbilt ¼-20 × 1\" ZINC combo truss-head machine screw into the tnut-quarter pronged tee-nuts — the machine-screw half of every ¼-20 ply-mount joint (36 + spare → 10× 4-pack = 40). Length 1\" gives full tee-nut engagement with ~3mm back protrusion. ZINC not SS (dry backboard mounts). Firm $1.57/4-pack (Home Depot 2026-08-07)."),
    ...
    Part("nut-m6-nyloc", "M6×1.0 hex nut, nyloc SS", "fasteners-hardware",
         "electrical", 4, "ea", 4.77 / 100, 4.77 / 100, "McMaster-Carr", part_no="90576A115", url="https://www.mcmaster.com/90576A115/", spec="Nyloc nut — electrical panel-mount bolts. Standardized off the former plain M6 nut (fastener-rework 2026-09-04): one M6 anti-vibration nut (nyloc) instead of plain + nyloc; transport-vibration-safe. Pitch M6×1.0 coarse — 90576A115. $4.77/pack of 100."),
    ...
    Part("fp-skate-axle-saddle", '1/8×3/4in 304 flat-bar axle saddles (skate) — 2 ft bar', "fasteners-hardware",
         "film", 2, "2 ft bar", 9.84, 9.84, "McMaster-Carr", part_no="8992K794",
         url="https://www.mcmaster.com/8992K794-8992K797/",
         spec='Axle retention — formed from 1/8" (3.18mm) × 3/4" (19mm) 304 SS flat bar, wrapped over each Ø10 axle (1mm cradle clearance) with two ~12mm feet bolted through the carriage plate (2× Ø6.5 M6). One saddle per axle, 4/skate ×4 = 16; 8 cut per 2 ft (610mm) length → 2 bars. Same stock + method as the spray skate. $9.84/2 ft bar (confirmed 2026-09-06).', dims="3.18×19 flat bar, ~48mm dev/pc", note="16 saddles = 2× 2 ft (610mm) 1/8×3/4 304 flat bar (8992K794); formed + drilled."),
    Part("bolt-m6x20", "M6×1.0 × 20 hex bolt, 304 SS (A2-70)", "fasteners-hardware",
         "film", 32, "ea", 17.77 / 50, 17.77 / 50, "McMaster-Carr", part_no="91287A137", url="https://www.mcmaster.com/91287A137/",
         spec="Skate axle-saddle retention — 2× M6 per saddle, 16 saddles = 32. Grip = 6mm carriage plate + 3.18mm saddle foot ≈ 9.2mm → M6×20 (nyloc under the plate). 304 SS A2-70 (wet cyanotype zone; no chloride → 316 unneeded). Consolidated onto the shared M6×20 — retires the former M5 (one M6 family, fastener-standardization). McMaster 91287A137 $17.77/pack of 50 firm."),
    Part("nut-m6-nyloc", "M6×1.0 hex nut, nyloc SS", "fasteners-hardware",
         "film", 32, "ea", 4.77 / 100, 4.77 / 100, "McMaster-Carr", part_no="90576A115", url="https://www.mcmaster.com/90576A115/",
         spec="Nyloc nut under the carriage plate — M6×20 skate axle-saddle screws (2/saddle ×16 = 32). Transport-vibration-safe. Pitch M6×1.0 coarse — 90576A115. $4.77/pack of 100."),
    ...
    Part("bolt-m8x25", "M8×1.25 × 25 hex bolt, 304 SS (A2-70) — cross-slide joints J1/J2", "fasteners-hardware",
         "film", 32, "ea", 10.60 / 25, 10.60 / 25, "McMaster-Carr", part_no="92314A711",
         url="https://www.mcmaster.com/92314A711/",
         spec="J1 (skate carriage → Z-slide) + J2 (X-way → Z-carriage) — 4× M8×25 each, 2 joints/corner ×4 = 32. THROUGH-BOLT + M8 nut (flat washer under head, split under nut), 24 N·m, Loctite 243 (Sheet 8). Length M8×25 for the 14mm J1 grip (6mm carriage plate + 8mm Z-carriage wall) + 2 washers + nut. 304 SS A2-70 (wet cyanotype zone; no chloride → 316 unneeded). McMaster 92314A711 $10.60/pack of 25 (sourced 2026-09-06)."),
    Part("washer-m8-flat", "M8 flat washer, 304 SS", "fasteners-hardware",
         ...
         spec="Flat washer under each J1/J2 M8×25 head (Sheet 8). 304 SS, wet zone. McMaster 98689A673 $13.59/pack of 50 (sourced 2026-09-06)."),
    Part("washer-m8-split", "M8 split lock washer, 304 SS", "fasteners-hardware",
         "film", 32, "ea", 11.19 / 100, 11.19 / 100, "McMaster-Carr", part_no="91111A132",
         url="https://www.mcmaster.com/91111A132/",
         spec="Split lock washer under each J1/J2 M8×25 (with Loctite 243 — belt + suspenders vs transport vibration; Sheet 8). 304 SS, wet zone. McMaster 91111A132 $11.19/pack of 100 (sourced 2026-09-06)."),
    ...
    Part("bolt-m4x12", "M4×0.7 × 12 hex bolt, 304 SS — cam-clamp base mount", "fasteners-hardware",
         "film", 24, "ea", 11.18 / 100, 11.18 / 100, "McMaster-Carr", part_no="91280A134",
         url="https://www.mcmaster.com/91280A134/",
         spec="Cam-clamp base → mount tab, BOLT-THROUGH (bolt + nut, not tapped) — 2× M4×0.7 per clamp, 3 clamps/corner ×4 = 24. The 5128A63 base has 2× M4×0.7 holes at ~15.7mm spacing. 304 SS (wet cyanotype zone). McMaster 91280A134 $11.18/pack of 100 (sourced 2026-09-06)."),
    Part("nut-m4-plain", "M4×0.7 hex nut, 304 SS", "fasteners-hardware",
         "film", 24, "ea", 7.25 / 100, 7.25 / 100, "McMaster-Carr", part_no="91828A231",
         url="https://www.mcmaster.com/91828A231/",
         spec="Plain hex nut under the cam-clamp mount tab — M4×12 bolt-through (2/clamp ×12 = 24). 304 SS, wet zone. Pitch M4×0.7. McMaster 91828A231 $7.25/pack of 100 (sourced 2026-09-06)."),
    Part("corner-l-plate", "Corner angle 304 SS (U-joint mount) — 4×4×¼in stock", "steel-structural",
         "film", 4, "ea", 25, 40, "Metal Supermarkets", "Online Metals", dims='4×4×¼in angle, ~152mm cut',
         spec='OFF-THE-SHELF 4"×4"×¼" 304 SS equal-leg ANGLE cut to ~152mm (6") + drilled — the frame-corner ↔ U-joint mount (was a press-brake-formed 6×8 plate 2026-09-05; stock angle gives the same ~4in legs with no forming, cheaper). Carries the concentrated U-joint corner load in STEEL, not aluminum; stainless for the cyanotype splash zone + galvanic match to the 303 SS U-joint. NOT expendable (the perimeter angle stays expendable 6061). Drill J5 (2× M6 to frame) + J4 (U-joint stub bore + 4040N12 mount). MATERIAL est $25–40/pc (a 6in cut of 304 4×4×¼ angle ≈ 0.5 ft; MetalsDepot ~$13.85/ft + cut) — re-quote at purchase; cheaper than the retired $58.90 formed plate.'),
    ...
    # ═══ lightlock (hinged-panel §8.2) — housing + drum + metal caps + lap joints + seals + handle;
    # sums to costing.LIGHTLOCK. Metal-cap / lap-joint / edge-channel / drum-brush redesign 2026-08 —
    # the Al-plate / rim-angle / DP8010 / edge-channel / stile prices are flagged ESTIMATES (firm at
    # the Aug-2026 re-price); rivets + drum-brush are confirmed SKUs. ═══
    Part("ll-hdpe-housing", "3/16\" UV-stab HDPE sheet, black — 48×96 (×3)", "plastics-sheet",
         ...
         spec="Ø800 fixed housing shell (~56 ft², ~2,062mm tall — spans beam-to-beam), rolled + extrusion-welded from 3× 4×8 ft 3/16\" (≈5mm) UV-stab HDPE sheets — the 99\" circumference needs 3× 48\" sheet widths (2 fall ~15\" short). ~33% offcut on the 3rd sheet; a 5×10 ft sheet would cut to 2 (optimize at order). US Plastics 46685 $184.99/sheet."),
    Part("ll-pp-drum", "1/8\" black HDPE sheet — 48×96 (×3)", "plastics-sheet",
         ...
         spec="Ø764 revolving drum shell (~49 ft², ~1,890mm tall) — rolled + extrusion-welded 1/8\" HDPE (94.5\" circ needs 3× 48\" widths). A light-tight cover over the steel/Al structure; its 1/8\" is non-structural (lighter = easier revolve, less bearing load). The 2 end caps are now 8mm 6061-T6 ALUMINUM (ll-caps-al) — NOT HDPE — since they carry the stub-shaft hubs into the SKF 6215 bearings. US Plastics 46684 $123.34/sheet."),
    Part("ll-caps-al", "8mm 6061-T6 aluminum plate — 2 drum caps (Ø755)", "aluminum",
         "lightlock", 1, "lot", 400, 700, "Online Metals", "Industrial Metal Supply",
         spec="Both drum end caps (8mm 6061-T6, Ø755, identical) — waterjet 2 discs + the 4×M10 hub bolt circle (Ø120 PCD) + the 280° rim-angle rivet holes. STRUCTURAL: carry the bolted stub-shaft flanges into the SKF 6215 bearings. Est. material + waterjet (Online Metals); firm at order.", note="PRICE-VERIFY (Aug-2026 re-price)"),
    Part("ll-rim-angle", "25×25×3 6061-T6 Al angle — 2 rim rings (rolled R427)", "aluminum",
         "lightlock", 1, "lot", 45, 90, "Online Metals", "Industrial Metal Supply",
         spec="2 rolled rim-angle rings (25×25×3 6061-T6, R427, ~5.4 m total) — the shell→cap lap-joint standing lip, riveted to both Al caps over the 280° arc. Material + roll. Est.; firm at order.", note="PRICE-VERIFY (Aug-2026 re-price)"),
    Part("ll-bearing-rings", "Bearing retaining rings + upper end-retainer plate", "fasteners-hardware",
         "lightlock", 1, "lot", 17, 18, "McMaster-Carr", part_no="90154A895", url="https://www.mcmaster.com/90154A895/",
         spec="Axial retention for the 2× SKF 6215. INNER race — EXTERNAL retaining rings on the Ø75 stub shafts (McMaster 90154A895, for OD 75mm, $7.05/pack — shaft groove Ø72.0 × 2.65mm, ring 2.5mm): 3 total = 2 on the LOWER shaft (both sides, floating bearing captive) + 1 drum-side on the UPPER shaft. The UPPER shaft's beam-side clip is REPLACED by an END-RETAINER PLATE (Ø90 × 4mm mild-steel disc, ≈$3 blank, central Ø10.5 c'bore) bolted by 1× M10×25 flat-head CSK screw (from the F1 CSK pack) into a tapped hole in the shaft end — its rim clamps the inner-race face so the drum's hanging load runs through a bolted member, not a lone circlip (transport-shock robust). OUTER race — 1× INTERNAL retaining ring in the Ø130 upper-ring bore (LOCATED upper bearing, beam side; McMaster 98455A170, for ID 130mm, $7.26/pack — bore groove Ø134.0 × 4.15mm, ring 4.0mm). The LOWER bearing FLOATS (plain Ø130 H7 bore, no ring). ≈$17 firm.", note="confirmed 2026-08-25 (SKUs 90154A895 + 98455A170, verified vs datasheets); +$3 end-retainer plate blank (upper hub) — 2026-08-25"),
    ...
    Part("ll-rivets-shell", "1/8\" 18-8 SS blind rivets — shell→cap lap (100-pack)", "fasteners-hardware",
         "lightlock", 1, "pack-100", 13.83, 13.83, "McMaster-Carr", part_no="97525A425",
         url="https://www.mcmaster.com/97525A425/",
         spec="Shell→cap lap rivets (~35/cap ×2 ≈ 70 → 1 pack of 100). 1/8\" (Ø3.18) 18-8 SS low-profile head, grip 0.188–0.25\", drill #30 (Ø3.3). Wet in DP8010 for light-tightness. $13.83/100 firm.", note="confirmed 2026-08-22 (SKU+URL+price)"),
    Part("ll-rivets-housing", "1/8\" 18-8 SS blind rivets — housing→frame lap (100-pack)", "fasteners-hardware",
         "lightlock", 1, "pack-100", 14.59, 14.59, "McMaster-Carr", part_no="97525A435",
         url="https://www.mcmaster.com/97525A435/",
         spec="Housing→frame lap rivets (~26/edge ×2 ≈ 52 → 1 pack of 100). 1/8\" (Ø3.18) 18-8 SS, grip 0.313–0.375\", drill #30. $14.59/100.", note="datasheet price 2026-08-22"),
    Part("ll-dp8010", "3M Scotch-Weld DP8010 structural adhesive (green, 45 mL)", "adhesives-finishes",
         "lightlock", 1, "ea", 76.29, 76.29, "McMaster-Carr", "3M", part_no="7467A36",
         url="https://www.mcmaster.com/7467A36/",
         spec="Structural LSE acrylic — bonds + light-seals the HDPE lap joints (shell→cap, housing→frame) and wets the open rivet mandrel bore for light-tightness. (The opening-edge channels are rivet-only as of 2026-09-08 — no DP8010.) 45 mL cartridge + mixing nozzle. $76.29 firm.", note="confirmed 2026-08-24 (SKU+URL+price)"),
    Part("ll-edge-channel", "Eagle Al H-divider — opening-edge H-mullions (×4)", "aluminum",
         "lightlock", 4, "12ft", 38, 38, "Eagle Aluminum", "Orange Aluminum", part_no="SGN-113",
         url="https://eagle-aluminum.com/product/aluminum-h-divider-1-38-for-316-material/",
         spec="4× Al H-mullion (Eagle Aluminum SGN-113 'H-divider 1-3/8\" for 3/16\" material', mill finish, 12 ft) — one at each of the 4 housing opening edges. The housing is built as TWO arc panels that SLIDE into the H slots (no welded seam): one slot takes the arc-panel edge (full height), the other the sill/header band edge (top+bottom). Riveted thru the flange + HDPE @120mm (Ø3.18 SS blind); mullion ends bolt via L-clip to the 4 support strips (ll-channel-strip; L-clips + M8 in the fasteners lot). One 12 ft stick per mullion (~82\" each). $38/12 ft. Confirm the alloy (likely 6063) + slot/leg depth on order.", note="confirmed 2026-09-09 (Eagle SGN-113 H-divider, $38/12 ft ×4)"),
    Part("ll-drum-channel", "6063-T5 Al U-channel — rotating-drum opening-edge stiffeners (×2) + rivets", "aluminum",
         "lightlock", 1, "lot", 30, 55, "Online Metals", "McMaster-Carr",
         spec="2× RIVETED Al U-channel (~10×15×2, ~2.1 m each ≈ 4.2 m) capping the ROTATING DRUM's two free HDPE opening-jamb edges (mirrors the housing opening channels but for the 3.18mm shell — rivet-only, no bond; legs lie flat so ~2mm radial proud clears the 13mm drum↔housing gap). Ø3.18 18-8 SS blind rivets @120mm thru both legs + shell (from the SS blind-rivet packs). Est.; confirm stocked section (2026-09-08).", note="PRICE-VERIFY (Aug-2026 re-price)"),
    Part("ll-channel-strip", "1×1×1/8in 6061-T6 Al angle — channel-support beams (×4) + rivets", "aluminum",
         "lightlock", 4, "4ft", 17.20, 17.20, "Metals Depot", "Online Metals",
         spec="4× 1\"×1\"×1/8\" (25×25×3.2) 6061-T6 aluminum equal-leg angle, ~1.2 m (4 ft) each (one strip per 4 ft piece) — one across each housing opening (top + bottom), spanning the full cage width (~962mm, rail-to-rail) so the ends land on the X-near/far frame rails, NOT floating. They give the two housing opening-edge U-channels (ll-edge-channel) a member to bolt to (1× M8/end via L-clip on the up-leg) at top and bottom, and are themselves Ø3.18 18-8 SS blind-riveted to the frame beams (flat leg on the beam; from the SS blind-rivet packs). Metals Depot 1×1×1/8 Al angle $17.20/4ft.", note="confirmed 2026-09-08 (metalsdepot.com 1×1×1/8 6061-T6 Al angle, $17.20/4 ft ×4)"),
    Part("ll-wiper-brush", "#4 (3/16\") black-nylon strip brush — running-gap light-seal wiper (×4 lines)", "seals-gaskets",
         "lightlock", 4, "8ft", 22, 40, "Gordon Brush", "Tanis Brush",
         url="https://www.gordonbrush.com/brushes/strip-brushes-holders/strip-brushes",
         spec="4 vertical #4 (3/16\") staple-set strip brushes — the running-gap light seal (drum↔housing). Metal channel backing, 0.008\" BLACK nylon, 0.687\" (17.5mm) trim; each snaps into an Al flange holder (ll-wiper-holder) that is flange-riveted to the rotating drum OD (rivets in the flange, clear of the brush — a 3/16\" channel is too small to rivet through). One 8 ft (2.44 m ≥ 1.97 m drum ht) piece per line → 4× 8ft. Est.; firm at order.", note="PRICE-VERIFY (brush/holder re-spec 2026-08-24)"),
    Part("ll-wiper-holder", "Anodized-Al straight-flange holder for the #4 wiper brush (×4 lines)", "aluminum",
         "lightlock", 4, "8ft", 18, 40, "Tanis Brush", "Gordon Brush", part_no="AH400436",
         url="https://www.tanisbrush.com/products/strip-brush/strip-brush-holders",
         spec="4× extruded anodized-aluminum straight-flange holders (0.050\" wall) for the #4 (3/16\") strip brush — the brush channel snaps in; the offset flat flange is DP8010-BONDED (primary — spreads the load over the whole flange so nothing pulls through the soft HDPE) + blind-riveted (with backup washers inside) to the drum OD, so fasteners land in the aluminum flange, clear of the brush. One 8 ft length per line → 4× 8ft. Exact profile P/N confirmed at order (Tanis/Gordon release the section via CAD/quote).", note="PRICE-VERIFY (brush/holder re-spec 2026-08-24)"),
    Part("ll-wiper-seal", "12mm closed-cell neoprene — top/bottom cap wiper seals", "seals-gaskets",
         "lightlock", 1, "lot", 25, 40, "Canal Rubber", "McMaster-Carr",
         spec="Top + bottom drum-cap wiper seals (12mm closed-cell neoprene, PSA back) + silicone bead to the frame plates. The drum↔housing running-gap seal is now the drum-mounted brush (ll-wiper-brush), not felt."),
    ...
    Part("ll-grab-rail", "12\" round pull handle — McMaster 1871A65", "fasteners-hardware",
         "lightlock", 1, "ea", 6.43, 6.43, "McMaster-Carr", part_no="1871A65",
         url="https://www.mcmaster.com/1871A65/",
         spec="Interior handle — off-the-shelf 12.13\" unthreaded-through-hole-mount round pull handle (Ø0.5\" bar, 2.06\" standoff, 1/4\" mounting screws). BOLTED at its two feet to the handle stile (ll-handle-stile) — 1/4\" screws into 1/4\"-20 rivet-nuts set in the hollow 5mm RHS wall (a blind insert gives full thread engagement + can't strip the thin wall) — NOT welded, and not to the drum wall. $6.43 firm.", note="confirmed 2026-08-24 (SKU+URL+price)"),
    Part("ll-handle-stile", "40×40×5 SS RHS pull-handle stile (cap→cap) + M12 cap bolts", "steel-structural",
         "lightlock", 1, "lot", 50, 95, "Metal Supermarkets", "Online Metals",
         spec="Steel stile (40×40×5 SS RHS, ~2.1 m) spanning + bolted between the two Al caps — carries the pull-handle load into the structural caps, NOT the thin HDPE wall. The open RHS is closed at each end by a SOLID TAPPED STEEL PLUG (2× ~30×30×40 blocks cut from bar, locked in the tube by 2× M8 cup-point grub/set screws each — anti-rotation); an M10 countersunk bolt driven from the cap's OUTSIDE face, through the cap into the tapped plug (sealed DP8010), secures each end — wrench-accessible, unlike a bolt buried in the tube. The off-the-shelf pull handle (ll-grab-rail, McMaster 1871A65) bolts to the stile (1/4\" screws into 1/4\"-20 rivet-nuts set in the hollow RHS wall). Est.; firm at order.", note="PRICE-VERIFY (Aug-2026 re-price)"),
    ...
    Part("ll-fasteners", "Bolts/nuts/isolation washers — cap/ring/collar/stile/handle/edge/housing", "fasteners-hardware",
         "lightlock", 1, "lot", 95, 95, "McMaster-Carr", part_no="91294A328", url="https://www.mcmaster.com/91294A328/",
         spec="18-8 SS, buy one pack per line (generous spares): F1 cap→flange 8× M10×1.5 CSK into the TAPPED steel flange — Ø11 clearance in the cap only, no nut (the flange is a machined part, so it is tapped directly on the Ø120 PCD; 91294A328 $11.58/25); F2/F3 ring/collar→MOUNT PLATE 14× M10×1.5 COUNTERSUNK flat-head TAPPED into the Ø240×12 steel mount plates — flush in the ring/collar underside (McMaster 91294A334, $5.22/10 → 2 packs $10.44; same CSK family as F1); F4 stile→cap 2× M10×1.5 CSK driven from the cap outside face THROUGH the cap into the TAPPED RHS-end plugs (from the F1 CSK pack) + 4× M8 cup-point GRUB/SET screws locking the plugs (anti-rotation — ~$8/pack, SKU to source); F5 pull-handle feet 4× 1/4\"-20 screws into 1/4\"-20 steel RIVET-NUTS (blind inserts set in the hollow 5mm RHS wall — full thread engagement, can't strip the thin wall; the 4 inserts hand-set with a draw bolt, no dedicated tool — ~$8/pack; screws 90272A537 $7.21/100); F6 edge-channel→beam 8× M8×1.25 (91280A083, $11.17/50); F7 housing→panel 8× M10×1.5 CSK (91280A370, $10.55/25) + 8× M10 nuts (90576A118, $9.53/50); W ~22× M10 nylon isolation shoulder washers (95610A400, $18.39/100) at every steel↔Al/HDPE joint. Blind rivets + the bearing mount plates are separate lines (the ring/collar rivet-nuts are GONE — they tap the 12mm plate directly). ≈$95 (F4 grub-screw SKU pending).", note="confirmed 2026-08-25 (F2/F3 91294A334); 2026-08-26 F2/F3 now TAP the Ø240×12 mount plate (rivet-nuts retired); F4→CSK cap-bolt + grub screws (stile-plug redesign), grub SKU pending; F1 cap→flange threads into the TAPPED flange (no nut); F5 handle feet → 1/4\" rivet-nuts in the hollow wall; 2026-09-07 M10 KEPT as the light-trap's single CSK size — the thin hosts (8mm cap, 5mm HDPE housing, 4mm bearing disc) preclude an M12 flat head, so this is a justified family (like M4), not an elimination target (see fastener-standardization.md)",),
    Part("ll-mount-plates", "Bearing mount plates — 2× Ø240×12 steel disc", "steel-structural",
         "lightlock", 2, "ea", 12, 22, "Online Metals", "SendCutSend",
         spec="Ø240 × 12mm A36 steel disc, one per hub — welded (fillet, both sides) across each 50×50 axle beam; the SKF 6215 ring/collar bolt (M10 tapped) into it, since the ring's Ø200 bolt circle is far wider than the 50mm beam (only 2 of 6 bolts would land on the beam). Waterjet/laser the disc + the Ø80 center clearance bore + the 6/8× M10 tap-drill holes on the Ø200 PCD; tap M10. ~3.4 kg ea. Est. material + cut (Online Metals plate + SendCutSend), firm at order.", note="NEW 2026-08-26 — replaces the ring/collar→beam rivet-nuts (retired); (mount plate to carry the wide bolt circle)"),
    Part("ll-holder-rivets", "1/8\" blind rivets + backup washers — brush-holder flange → drum OD", "fasteners-hardware",
         "lightlock", 1, "lot", 16, 16, "McMaster-Carr", part_no="97447A015", url="https://www.mcmaster.com/97447A015/",
         spec="~72 rivets (4 brush-holder lines × ~18 @ 120mm) fastening the Al holder flanges to the rotating drum OD — grip ~4.5mm (1.27mm Al flange + 1/8\" HDPE). Blind rivets (97447A015, $10.78/250) with Ø1/8\" BACKUP WASHERS on the blind (inside) face (~$5/pack) so the bulb can't pull through the soft HDPE. The rivets are the mechanical backup — the flange is PRIMARY-BONDED to the shell with DP8010 (existing ll-dp8010 adhesive), which spreads the load over the whole flange. ≈$16 firm.", note="confirmed 2026-08-24 (rivet SKU); +$5 backup washers for the HDPE — 2026-08-25"),
    Part("ll-rim-rivets", "1/8\" 18-8 SS blind rivets — housing skin → outer beam face (100-pack)", "fasteners-hardware",
         "lightlock", 1, "pack-100", 13.83, 13.83, "McMaster-Carr", part_no="97525A425", url="https://www.mcmaster.com/97525A425/",
         spec="~24× 1/8\" (Ø3.18) 18-8 SS blind rivets fastening the housing skin's top + bottom edges DIRECTLY to the steel frame top + bottom beams (2× 100° arcs each, ~150mm pitch). The skin now extends to the OUTER beam face and fixes straight to the beam — the rolled rim-angle is dropped (2026-09-08). A blind rivet suits the CLOSED 3mm RHS beam: the set head forms inside the bore (no internal access) and bears on the full wall, vs only ~2 threads for a self-driller; 18-8 SS bites the 3mm steel far better than an aluminum rivet. Grip 0.188–0.25\" (4.8–6.35mm) fits the 5mm HDPE skin + 3mm steel wall; drill #30 (Ø3.3). Same SS rivet as the shell→cap lap (97525A425). $13.83/100 firm.", note="confirmed 2026-08-25 (97525A425 = 18-8 SS per eng-specs datasheet); 2026-09-08 rim-angle dropped — skin fixes directly to the outer beam face"),
    Part("ll-fabrication", "Fabrication — roll + weld 2 HDPE cylinders, roll rim-angle, fit metal caps/bearings", "fabrication-labor",
         "lightlock", 1, "lot", 800, 1150, "Local plastic + metal fab", spec="~16–22 hrs: roll + hot-air/extrusion-weld the 2 HDPE cylinders, roll the Al rim-angle rings, fit the metal caps + SKF 6215 bearings + stub shafts, rivet the lap joints, rivet the 4 channel-support strips to the frame beams, mount the drum brush holders + edge channels + pull-handle stile."),
    ...
    Part("sp-wall-stays", "Top + bottom wall stays + 4-bolt anchor plates", "fasteners-hardware",
         "swing", 2, "set", 45, 60, "Fasteners Plus", spec="Transport lock — 2× M16 jaw-eye turnbuckle + 2 panel-side welded eye lugs (12mm steel, Ø17 clevis-pin hole, fillet-welded to the stile) + inside/outside wall anchor plates (200×200×12 A36 pair, 4× M16 through-bolts per anchor); hingepanel Sheet 17", part_no="JETBGV58X6", url="https://www.fastenersplus.com/products/5-8-x-6-jaw-eye-galvanized-turnbuckle"),
    ...
    Part("sp-door-frame-rhs", "50×20×3 RHS (6 m bulk lengths)", "steel-structural",
         "door", 3, "ea", 30, 40, "Metal Supermarkets", spec="Frame members — 50×20×3 RHS (50mm FACE = the seal landing, 20mm depth). REUSES the IBC retaining-bar section (ibcf-rhs, 50×20×3) so it needs no bespoke order. Non-weight-bearing (the pivot post carries the hinge panel) — the frame only takes seal-compression + cam-latch load."),
    ...
    Part("panel-cam-latch", "Lift-and-turn tight-hold cam latch (McMaster 1619A74)", "fasteners-hardware",
         "panel", 2, "ea", 30.57, 30.57, "McMaster-Carr", part_no="1619A74", url="https://www.mcmaster.com/1619A74/",
         spec="TWO interior-mounted latches on the OPENING (free) edge of the swinging panel — top + bottom — clamping it to the fixed door frame (the pivot edge is hinged; a frame stop takes the outward direction, so only the free edge needs latching). Turns from the INTERIOR face; the reversible-offset cam hooks a welded steel keeper on the stub-wall steel frame outside, drawing the panel onto the 20mm EPDM seal (panel opens INWARD only). 150 lbf max clamp; ADJUSTABLE latching distance 1-3/4 to 2-1/8in (44-54mm); 1/4in max panel thk; 11/16in through-hole; black-painted zinc; mounting fasteners incl. Lift-and-turn = vibration-resistant for transport + hand-releasable from inside for egress. Mounts through the panel-frame RHS; the latch nut is tightened via a coaxial over-bore in the far tube wall + box spanner, then a light-tight plug (or a welded tapped boss — fab detail on Sheet 12). $30.57 ea firm.", note="NEW 2026-08-28 — replaces the recessed Southco C2-33; qty 2 (opening edge only, was 4 corners); adjustable 150 lbf cam latch, welded stub-wall keeper"),
    Part("panel-b2-bay", "1/8\" black HDPE sheet (48×96, ×2)", "plastics-sheet",
         ...
         url="https://www.usplastic.com/catalog/item.aspx?itemid=136961&catid=705",
         spec="B2 punch-out bay — 4-wall light-tight tube (~890mm deep) around the housing; 4 walls, 2 per 4×8 sheet. 1/8\" HDPE nearest stock to 4mm (weld-compatible with the HDPE housing/drum); EPDM lip cut from the panel-epdm perimeter roll (not billed here). US Plastics 46684 $123.34/sheet."),
    Part("panel-surround-rivets", "1/8\" 18-8 SS blind rivets — HDPE surround → frame lap (100-pack ×2)", "fasteners-hardware",
         "panel", 2, "pack", 14.59, 14.59, "McMaster-Carr", part_no="97525A435", url="https://www.mcmaster.com/97525A435/",
    ...
    Part("shelf-piano-hinge", "Extra-clearance continuous (piano) hinge, 600 mm, SS", "fasteners-hardware",
         "shelf", 1, "ea", 55.58, 55.58, "McMaster-Carr", part_no="14835A77", url="https://www.mcmaster.com/14835A77-14835A33/", spec="304 SS extra-clearance continuous hinge, 76.2mm open / 38.1mm leaf, Ø10.16mm barrel, 0.075in thick (14835A33 datasheet), 3ft stock cut to 600, supplied BLANK (undrilled) — drill both leaves to the 150mm pitch (4 bolts), the row 25mm off the edge (mid-leaf) so the hinge, the ply tee-nuts and the cleat all align. The 3in-open leaf lets the tee-nut row sit at a full 25mm ply edge distance (flange fully on-board). Shelf leaf 1/4-20 SS machine-screwed (pan/truss head) into the ply tee-nut row; wall leaf tapped 1/4-20 into the 6mm cleat. $55.58/3ft (14835A77) — upsized from the 1in-open 1582A457 so the plywood tee-nut gets real edge distance."),
    Part("shelf-chain", "304 SS chain — 2 tension stays", "fasteners-hardware",
         "shelf", 1, "3ft", 23.79, 23.79, "McMaster-Carr", part_no="3392T51", url="https://www.mcmaster.com/3392T51-3392T512/", spec="304 SS ~4mm chain, 2 tension stays (front corner → wall anchor ~230mm above the hinge), ~1m used of the 3ft; trimmed to set the deployed level. WLL >> the ~91N tension demand (chem_shelf_load.py SF 5.4). $23.79/3ft."),
    Part("shelf-eye-bolt", "1/4\"-20 SS eye bolt — shelf front-corner chain anchor", "fasteners-hardware",
         "shelf", 2, "ea", 4.16, 4.16, "McMaster-Carr", part_no="3014T45", url="https://www.mcmaster.com/3014T45/", spec="304 SS 1/4-20 eye bolt into a ply tee-nut at each front corner (~1in shank; tip protrudes ~7mm through the 18mm top — cap/grind at the corner). Chain hooks to the eye. $4.16 ea."),
    Part("shelf-wall-eye-bolt", "M8 SS eye bolt — chain wall anchor", "fasteners-hardware",
         "shelf", 2, "ea", 7.80, 7.80, "McMaster-Carr", part_no="4843T13", url="https://www.mcmaster.com/4843T13/", spec="304 SS M8 eye bolt into the tapped M8 backing plate ~230mm above the hinge (reuses the M8 wall standard) — the chain's top anchor. $7.80 ea."),
    Part("shelf-quick-links", "304 SS quick-links", "fasteners-hardware",
         "shelf", 4, "ea", 2.90, 2.90, "McMaster-Carr", part_no="8947T25", url="https://www.mcmaster.com/8947T25/", spec="304 SS ~4mm quick-links — chain ends + length adjustment (2 per stay). $2.90 ea."),
    Part("shelf-tnut", "1/4\"-20 pronged tee-nut (ply attach)", "fasteners-hardware",
         "shelf", 2, "4-pack", 1.57, 1.57, "Home Depot", part_no="825001", url="https://www.homedepot.com/p/Everbilt-1-4-in-20-Zinc-Plated-Tee-Nut-4-Pack-825001/317478995", spec="1/4-20 4-prong tee-nut (the ply-mount standard SKU 825001) seated in the ply underside — the 4-bolt hinge row + 2 front-corner eye bolts = 6 → 2× 4-pack (8, with spares). Reuses the project ply-mount tee-nut; replaces the retired M5 CSK ply attach."),
    Part("shelf-hinge-screws", "1/4\"-20 SS machine screws — hinge leaves (both)", "fasteners-hardware",
         "shelf", 8, "ea", 0.40, 0.60, "McMaster-Carr", spec="1/4-20 SS machine screws — BOTH hinge leaves at 4 each (Sheet 5 drilling detail): 4× ~3/4in shelf leaf → the ply tee-nut row (board); 4× ~5/16in wall leaf → the TAPPED 6mm cleat (short, no protrusion past the cleat back onto the backing plate). SS is the splash-zone exception to the zinc ply-mount screw (panel-machine-screws). ~8 off. SKU pending — source 1/4-20 SS pan/truss-head machine screws (they seat flat on the leaf, into the flanged tee-nut / tapped cleat)."),
    Part("shelf-wall-cleat", "Hinge wall cleat, 6mm steel", "steel-structural",
         "shelf", 1, "lot", 10, 10, "Local fab", spec="6mm steel cleat behind the piano-hinge wall leaf (bolts to the backing plate via M8). The 2 chain wall anchors are M8 eye bolts into the backing plates, not slotted stay anchors."),
    Part("shelf-wall-backing", "Shelf mount backing plates, 8mm steel (welded, ×3)", "steel-structural",
         "shelf", 3, "ea", 6, 10, "Local fab", spec="Flat 8mm steel backing plates welded to the pinhole-wall interior crests — one behind the hinge cleat + one per chain wall-anchor (M8 eye bolt) — flat, solid load anchors, TAPPED M8 (8mm plate taps directly — no weld-nut, no loose nut behind the wall-welded plate)."),
    Part("bolt-m8-wall", "M8×1.25 × 25 hex bolt, Grade 8.8 zinc — shelf hinge cleat", "fasteners-hardware",
         "shelf", 4, "ea", 18.51 / 50, 18.51 / 50, "McMaster-Carr", part_no="91280A534", url="https://www.mcmaster.com/91280A534/", spec="Clamps the shelf hinge cleat (6mm) to its welded, TAPPED 8mm backing plate — 1 per hinge bolt (4 at the 150mm hinge pitch). The 8mm plate taps M8 directly (no weld-nut, no loose nut — the plate is wall-welded, no access behind). Grip = 6mm cleat, thread engages the plate. M8×25, pitch M8×1.25 coarse. $18.51/pack of 50 (same 91280A534 as the film ICP-14 fixing)."),
    Part("washer-m8-flat-shelf", "M8 flat washer, SS", "fasteners-hardware",
         "shelf", 4, "ea", 3.32 / 100, 3.32 / 100, "McMaster-Carr", part_no="91166A270", url="https://www.mcmaster.com/91166A270/", spec="Flat washer (1/bolt) — shelf hinge-cleat bolts (dry). Distinct SKU from the film-plane wet-zone washer-m8-flat (98689A673): separate keys so the by-type BOM doesn't blend the two M8 flat washers. $3.32/pack of 100."),
    Part("shelf-transport-latch", "Transport latch — cam latch (reuse 1619A74)", "fasteners-hardware",
         "shelf", 1, "ea", 30.57, 30.57, "McMaster-Carr", part_no="1619A74", url="https://www.mcmaster.com/1619A74/", spec="Secures the folded board for transport — reuses the hinged-panel cam latch (1619A74), a standard-part reuse that retires the est zinc toggle. $30.57 ea."),
    Part("shelf-edge-sealer", "Ply edge sealer + lip fixings", "adhesives-finishes",
         "shelf", 1, "lot", 10, 10, "Hardware store", spec="Seal the ply cut edges (chemical-resistant) + fix the spill lip (ply/HDPE offcut, 3 free edges). No steel frame to powder-coat (frame removed)."),
    ...
    Part("walkway-std-brackets", "Cantilever bracket — standard (near/far)", "steel-structural",
         ...
         spec="8mm steel plate (180mm vert leg + 70mm gusset) + a 300mm 2×1×0.120in steel tube arm, welded — REDESIGNED to US IBC/OSHA (60 psf + 300 lbf concentrated, IBC Table 1607.1): the old 8mm plate arm yielded at ~25 lbf; the 2×1 tube arm carries the 300 lbf tip load at SF 2.10 (walkway_load.py). 4 near + 9 far at 457mm centers. ~0.3 m of 2×1 tube/bracket (same stock as the floor-leg + RWK arms)."),
    ...
    Part("walkway-corner-plates", "Combined corner plate (right corners)", "steel-structural",
         ...
         spec="10mm steel, ~200mm wide (widened inboard to back the walkway right beam) — carries the walkway right beam AND the bottom film rail"),
    Part("bolt-m12x70", "M12×70 hex through-bolt, Grade 8.8 zinc, partial-thread", "fasteners-hardware",
    ...
    Part("walkway-arm-holddown", "Half-lap hold-down screws (#14 TEK + washer)", "fasteners-hardware",
         "ibc-frame", 4, "ea", 12.88 / 25 + 13.48 / 100, 12.88 / 25 + 13.48 / 100, "McMaster-Carr", part_no="90822A620", url="https://www.mcmaster.com/90822A620/",   # arm half-lap fixing — IBC-owned, reconciled from 'walkway' 2026-08-19
         spec="1× #14 self-drilling TEK screw + SS flat washer per half-lap (4 total: 2 arms × 2 crossings) — driven from the underside through a Ø7 clearance hole in the long beam, self-tapping into the SOLID arm above, so the beam is positively secured to the bearing seat (anti-lift/anti-slide). Head clears the traveling spray beam (~6.6mm). The SAME #14×1 HWH 410-SS TEK as the J8/J9 bracket screws (McMaster 90822A620 $12.88/25) + a 1/4 SS flat washer (McMaster 92141A223 $13.48/100) under each head. Firm 2026-09-07."),
    Part("nut-m12-plain", "M12 hex nut, plain", "fasteners-hardware",
    ...
    # ═══ front-board (tilt-swing-board-report.md §12) — the interchangeable tilt-swing pinhole plate.
    # Standalone module (not one of the numbered SECTIONS); reconciles to EXPECTED['front-board']. Shop/
    # custom items carry "SKU pending — source" (raw material + fab/finishing services still to be quoted). ═══
    Part("tsb-wave-spring", "Annular wave spring — carrier preload (ICP-03)", "bearings-motion",
         "front-board", 1, "ea", 25, 50, "Smalley", "Associated Spring",
         spec="ICP-03 preload: ~Ø300 mean-dia crest-to-crest wave spring (17-7 PH SS), seated in a counterbore in ICP-01's interior face; bears on the carrier's exterior rim to push it onto the 4 adjuster balls (zero backlash). Replaces the former central GE50 bearing — the carrier is rim-supported and the optical axis is clear. Large-dia custom coil — SKU pending (Smalley / Associated Spring)."),
    Part("tsb-adjuster-ring", "Adjuster bracket ring — Al (ICP-03)", "aluminum",
         "front-board", 1, "ea", 35, 70, "local machine shop",
         spec="ICP-03: 6061-T6 annular bracket ring ~Ø470 OD × Ø250 ID × 8mm, standoff-mounted to ICP-01's interior face (6× M5 @ Ø450, outside the bellows). Carries the 4 M8 adjuster bushings at Ø270 with the KNOBS facing into the container (adjust from inside) and reacts the wave-spring preload. Waterjet blank + machined (bushing bores + tapped mounts). SKU pending — fab quote."),
    Part("tsb-ring-screws", "M5×30 SHCS 18-8 SS (bracket-ring standoff)", "fasteners-hardware",
         "front-board", 1, "pack", 8, 8, "McMaster-Carr", "Bolt Depot",
         spec="6 off — standoff screws mounting the adjuster bracket ring to ICP-01 at Ø450 (tapped). M5×30 SHCS 18-8 SS, pack of 25 — SKU pending (source)."),
    Part("tsb-icp01-plate", "ICP-01 outer adapter frame stock (6061-T651 plate)", "aluminum",
         "front-board", 1, "ea", 125, 125, "Online Metals", "Metal Supermarkets",
         spec="6061-T651 plate 620×620×45mm (24×24×1.75in) — machined to the 600×600×40 outer frame. SKU pending — source (Online Metals / Metal Supermarkets, Chatsworth)."),
    Part("tsb-icp02-bar", "ICP-02 inner carrier stock (6061-T6 round bar)", "aluminum",
         "front-board", 1, "ea", 100, 100, "Metal Supermarkets", "Online Metals",
         spec="6061-T6 round bar Ø340×30mm (13.5in OD × 1.2in) — machined to the Ø320×25 carrier (no central shank — rim-supported). SKU pending — source (Metal Supermarkets, Chatsworth / Online Metals)."),
    Part("tsb-adj-screw", "M8×1.0×80 SHCS 18-8 SS (adjustment screws)", "fasteners-hardware",
         "front-board", 4, "ea", 1.873, 1.873, "McMaster-Carr", "Bolt Depot", part_no="91180A407", url="https://www.mcmaster.com/91180A407/",
         spec="Fine-pitch adjustment screw, ball-end seats in the 440C insert; partially threaded. 4 off. McMaster 91180A407 $18.73/10."),
    Part("tsb-delrin-bushing", "Delrin/POM guide bushing rod", "plastics-sheet",
         "front-board", 1, "ea", 20, 20, "McMaster-Carr", "Amazon Industrial", part_no="8573K75", url="https://www.mcmaster.com/8573K75/",
         spec="Ø30×200mm Delrin/POM rod — machine the 4 adjustment-screw guide bushings (pressed into the adjuster bracket ring, not the frame). McMaster 8573K75."),
    Part("tsb-chrome-balls", "Ø8mm Grade-25 chrome steel balls (10-pack)", "bearings-motion",
         "front-board", 1, "pack", 14, 14, "McMaster-Carr", "Precision Balls Inc.", part_no="9528K22", url="https://www.mcmaster.com/9528K22/",
         spec="52100 bearing steel, Ø8mm Grade 25, 10-pack — the ball-end contact at each adjustment screw. McMaster 9528K22."),
    Part("tsb-lock-setscrew", "M6×1.0 nylon-tip set screw (10-pack)", "fasteners-hardware",
         "front-board", 1, "pack", 14, 14, "McMaster-Carr", "Fastenal", part_no="91375A187", url="https://www.mcmaster.com/91375A187/",
         spec="SS316, M6×20mm nylon-tip — cross-locks each adjustment screw for long exposures. Pack of 10. McMaster 91375A187."),
    Part("tsb-440c-bar", "440C SS round bar (ball-socket inserts)", "stainless-sheet",
         "front-board", 1, "ea", 28, 28, "McMaster-Carr", "Metal Supermarkets", part_no="1765T17", url="https://www.mcmaster.com/1765T17/",
         spec="Ø20×100mm 440C SS bar — machine the 4 hardened kinematic-seat inserts (ICP-05): 1 cone / 1 V-groove / 2 flat, Ra 0.4 ground (locate the carrier in-plane + anti-spin). McMaster 1765T17."),
    Part("tsb-m12-bolt", "M12×45 SHCS SS A4 (plate-to-frame)", "fasteners-hardware",
         "front-board", 8, "ea", 4, 4, "McMaster-Carr", "Bolt Depot", part_no="92290A198", url="https://www.mcmaster.com/92290A198/",
         spec="8 off — mounts the TSB into the same wall frame as the standard plate (torque 65 Nm). McMaster 92290A198 $20/pack of 5."),
    Part("tsb-dowel-pin", "Ø8 m6 SS303 dowel pin", "fasteners-hardware",
         "front-board", 2, "ea", 9, 9, "McMaster-Carr", "Fastenal", part_no="97395A437", url="https://www.mcmaster.com/97395A437/",
         spec="Ø8×40mm — plate registration repeatability (light-pull release). 2 off. McMaster 97395A437."),
    Part("tsb-loctite638", "Loctite 638 retaining compound (10mL)", "adhesives-finishes",
         "front-board", 1, "ea", 22, 22, "McMaster-Carr", "Home Depot", part_no="1832A1", url="https://www.mcmaster.com/1832A1/",
         spec="Kinematic-seat insert (ICP-05) retention in the carrier bores. McMaster 1832A1."),
    Part("tsb-bellows", "Custom photographic bellows (ICP-04)", "seals-gaskets",
         "front-board", 1, "ea", 80, 150, "Micro-Tools", "Ames Camera Repair",
         spec="Truncated cone Ø290 (carrier) → Ø430 (frame) × 60mm free length, 4-pleat, matte-black neoprene — the primary zero-friction light seal; clamp-ring retained both ends. Custom order — SKU pending (Micro-Tools / Ames Camera Repair)."),
    Part("tsb-neoprene-cord", "Neoprene cord seal Ø3mm", "seals-gaskets",
         "front-board", 1, "ea", 18, 18, "McMaster-Carr", "Grainger", part_no="1834K22", url="https://www.mcmaster.com/1834K22/",
         spec="70 Shore, 1.5m — bellows clamp-ring gaskets (carrier + frame flange) + Ø420 loop (same spec as the wall-frame seal). McMaster 1834K22."),
    Part("tsb-clamp-rings", "Bellows clamp rings — Al retaining (inner + outer)", "aluminum",
         "front-board", 2, "ea", 12, 20, "local machine shop",
         spec="2× Al retaining ring clamping the bellows flanges onto the cord gasket — inner ring ~Ø296 (carrier), outer ring ~Ø434 (frame, outside the labyrinth). Waterjet blank + light machining. SKU pending — fab quote."),
    Part("tsb-clamp-screws", "M4×12 SHCS 18-8 SS (bellows clamp-ring)", "fasteners-hardware",
         "front-board", 1, "pack", 6, 6, "McMaster-Carr", "Bolt Depot",
         spec="Retains the 2 bellows clamp rings — 4× on the carrier ring + 6× on the frame ring = 10 off, pack of 25. M4×12 SHCS 18-8 SS — SKU pending (source)."),
    Part("tsb-anodise-hard", "Hard anodize — ICP-01 exterior (service)", "adhesives-finishes",
         "front-board", 1, "job", 80, 120, "Pac-Nor Anodizing",
         spec="MIL-A-8625 Type III, 0.025mm, ICP-01 exterior. SKU pending — get a shop quote (Pac-Nor, Chatsworth)."),
    Part("tsb-anodise-black", "Black anodize — ICP-02 + knobs (service)", "adhesives-finishes",
         "front-board", 1, "job", 60, 90, "Aero Finishing",
         spec="Type II, ICP-02 carrier + the 4 knobs. SKU pending — get a shop quote (Aero Finishing, Burbank)."),
    Part("tsb-cnc", "CNC machining — ICP-01 + ICP-02 (service)", "fabrication-labor",
         "front-board", 1, "job", 800, 1500, "Fictiv", "ProtoLabs",
         spec="All-aluminum machining of both plates + the adjuster bracket ring (Ø380 bore, wave-spring counterbore + bracket-ring standoff taps on the frame, kinematic-seat insert bores on the carrier rear face, PCDs, labyrinth, counterbore). SKU pending — get a fab quote (Fictiv / ProtoLabs)."),
    Part("tsb-knob-stock", "Knurled knob stock Ø40mm Al (4 off)", "aluminum",
         "front-board", 4, "ea", 15, 15, "Jergens", "local machine shop",
         spec="Ø40mm Al knurled knob — 4 off (2 black TILT, 2 silver SWING), engraved. Jergens 49525 (SKU pending — verify) or machine from bar."),
    Part("tsb-scale-engrave", "Angular calibration scale engraving (service)", "fabrication-labor",
         "front-board", 1, "set", 35, 50, "LaserPros",
         spec="Al 80×15×2mm, 2 off — laser-engraved angular scales (tilt + swing). SKU pending — quote (LaserPros, Chatsworth)."),
]
...
SYSTEM_DOC = {
    ...
    "front-board": "tilt-swing-board-report.md",
}
