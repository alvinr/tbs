...
PARTS: list[Part] = [
    ...
    Part("s60-adapter", 'S60×6 to 1" NPT adapter', "plumbing-fittings",
         "water", 8, "ea", 8, 15, "JME Sales", "Amazon", spec='IBC DN50 valve to 1" HDPE; PP S60×6 male × 1" NPT female'),
    ...
    Part("bolt-m6-tray", "M6 hex bolt, SS — ~M6×16 est (tray center flange)", "fasteners-hardware",
         "tray", 12, "ea", 0.92, 0.92, "McMaster-Carr", spec="Tray center-flange bolts (316 SS, wet zone). Grip ≈ 3mm (2× 1.5mm SS flange) + silicone gasket → ~M6×16, but the joint is BOM-only (not drawn) + gasket thickness unknown — see TODO."),
    ...
    Part("bolt-m6-pinch", "M6 hex bolt, SS — length BLOCKED (pinch-clamp geometry undesigned)", "fasteners-hardware",
         "spray", 1, "ea", 0.94, 0.94, "McMaster-Carr", spec="Pinch bolt for the spray arm tube (Ø25×2 6061). Length CAN'T be derived — the pinch-clamp ear/slit geometry was never designed or drawn (see TODO). (316 SS)"),
    Part("nut-m6-plain", "M6×1.0 hex nut, plain SS", "fasteners-hardware",
         "spray", 1, "ea", 3.42 / 100, 3.42 / 100, "McMaster-Carr", part_no="90591A151", url="https://www.mcmaster.com/90591A151/", spec="Plain hex nut — spray pinch bolt. Pitch M6×1.0 (coarse, baseline — confirm vs SKU PDF, must match the bolt). $3.42/pack of 100."),
    ...
    Part("bolt-m6-panel", "M6 hex bolt — M6×20 (crest-seated) to M6×55 (corrugation-bridge), confirm mount", "fasteners-hardware",
         "electrical", 4, "ea", 1.13, 1.13, "McMaster-Carr", spec="External flush power-panel bolt (A2/304 SS) through the pinhole END wall. Grip = 3mm plate + 3mm gasket + wall → ~8mm/M6×20 if crest-seated (flat backing), up to ~42mm/M6×55 if it bridges the 36mm end-wall corrugation — needs the mount detail (see TODO)."),
    Part("nut-m6-plain", "M6×1.0 hex nut, plain SS", "fasteners-hardware",
    ...
    Part("corrugated-conduit", "10mm corrugated conduit, drop runs", "electrical-distribution",
         "electrical", 10, "m", 3, 3, "McMaster-Carr",
         note="Auto-guessed SKU 7828K48 was invalid (does not exist) — pin a real 10mm split/corrugated wire-loom SKU + URL at order."),
    ...
    Part("bolt-m8-wall", "M8 hex bolt, SS — M8×20-25 (rib-flat) to longer (corrugation-bridge), confirm attach", "fasteners-hardware",
         "shelf", 12, "ea", 0.84, 0.84, "McMaster-Carr", spec="Shelf hinge cleat + 2 stay anchors, through-bolted (12 M8 nuts) to the pinhole END wall. Grip = 6mm cleat + wall → ~8mm/M8×20-25 if landing on a rib/flat backing, longer if bridging the end-wall corrugation — needs the attachment mode (see TODO)."),
    Part("nut-m8-plain", "M8×1.25 hex nut, plain SS", "fasteners-hardware",
