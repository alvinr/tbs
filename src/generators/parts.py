...
PARTS: list[Part] = [
    ...
    Part("ll-edge-channel", "6063-T5 Al U-channel opening-edge stiffeners (×4) + L-clips", "aluminum",
         "lightlock", 1, "lot", 55, 110, "Online Metals", "McMaster-Carr",
         spec="4× RIVETED Al U-channel (~20×18×3, ~2.1 m each ≈ 8.5 m) stiffening the housing's two free HDPE opening edges — replaces the steel jamb posts — + 8× L-clip + 8× M8 end bolts (channel ends bolt via L-clip to the 4 formed-Al support strips, ll-channel-strip). RIVET-ONLY as of 2026-09-08 (Ø3.18 SS blind @120mm thru both legs + HDPE; no DP8010 — the opaque channel wraps the edge). Est.; confirm stocked section.", note="PRICE-VERIFY (Aug-2026 re-price)"),
    ...
    Part("ll-channel-strip", "Formed-Al strip channel-support beams (×4) + rivets", "aluminum",
         "lightlock", 1, "lot", 20, 45, "Online Metals", "McMaster-Carr",
         spec="4× formed-aluminum strip beams (smallest stocked formed section — SKU pending, sourcing) — one across each housing opening (top + bottom), spanning the full cage width (~962mm, rail-to-rail) so the ends land on the X-near/far frame rails, NOT floating. They give the two housing opening-edge U-channels (ll-edge-channel) a member to bolt to (1× M8/end via L-clip) at top and bottom, and are themselves Ø3.18 18-8 SS blind-riveted to the frame beams (from the SS blind-rivet packs). Est.; confirm section at order (2026-09-08).", note="SKU pending — smallest formed section (sourcing); PRICE-VERIFY (Aug-2026 re-price)"),
    ...
    Part("panel-bottom-apron", "Fold-down light aprons + fixed baffle/pivot stub (12mm ext ply) + piano hinges + foam seal + hold-up catches", "timber-ply",
         ...
         note="NEW 2026-08-30 (Alvin) — fold-down light apron"),
