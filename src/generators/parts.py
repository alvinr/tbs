...
PARTS: list[Part] = [
    ...
    Part("ll-handle-stile", "40×40×5 SS RHS pull-handle stile (cap→cap) + M12 cap bolts", "steel-structural",
         ...
         spec="Steel stile (40×40×5 SS RHS, ~2.1 m) spanning + bolted between the two Al caps — carries the pull-handle load into the structural caps, NOT the thin HDPE wall. The open RHS is closed at each end by a SOLID STEEL PLUG (2× ~30×30×40 blocks cut from bar, cross-bolted through the tube walls with 2× M8 each) that gives the tube a bolting face; a single M12 tapped into each cap clamps the plug up to the cap (blind, ~8mm engagement — no pierce). The off-the-shelf pull handle (ll-grab-rail, McMaster 1871A65) bolts to the stile (1/4\" screws tapped into the RHS wall). Est.; firm at order.", note="PRICE-VERIFY (Aug-2026 re-price)"),
