...
PARTS: list[Part] = [
    ...
    Part("ll-grab-rail", "12\" round pull handle — McMaster 1871A65", "fasteners-hardware",
         ...
         spec="Interior handle — off-the-shelf 12.13\" unthreaded-through-hole-mount round pull handle (Ø0.5\" bar, 2.06\" standoff, 1/4\" mounting screws). BOLTED at its two feet to the handle stile (ll-handle-stile) — 1/4\" screws tapped into the RHS wall — NOT welded, and not to the drum wall. $6.43 firm.", note="confirmed 2026-08-24 (SKU+URL+price)"),
    Part("ll-handle-stile", "40×40×5 SS RHS pull-handle stile (cap→cap) + M12 cap bolts", "steel-structural",
         ...
         spec="Steel stile (40×40×5 SS RHS, ~2.1 m) spanning + bolted between the two Al caps — carries the pull-handle load into the structural caps, NOT the thin HDPE wall. The open RHS is closed at each end by a SOLID TAPPED STEEL PLUG (2× ~30×30×40 blocks cut from bar, locked in the tube by 2× M8 cup-point grub/set screws each — anti-rotation); an M10 countersunk bolt driven from the cap's OUTSIDE face, through the cap into the tapped plug (sealed DP8010), secures each end — wrench-accessible, unlike a bolt buried in the tube. The off-the-shelf pull handle (ll-grab-rail, McMaster 1871A65) bolts to the stile (1/4\" screws tapped into the RHS wall). Est.; firm at order.", note="PRICE-VERIFY (Aug-2026 re-price)"),
