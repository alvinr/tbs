...
PARTS: list[Part] = [
    ...
    Part("led-channel", "Aluminum LED channel + frosted diffuser, surface-mount (~10 ft)", "electrical-distribution",
         "electrical", 1, "lot", 25, 45, "Amazon", "Muzata",
         spec="Surface-mount aluminum U-channel (inner slot ≥ strip PCB width — HitLights 12V COB is 8mm → an 8–10mm channel) with snap-in FROSTED/OPAL diffuser + end caps + mounting clips. ~10 ft total to house the Circuit-G COB strip run (sold in 1m/2m sticks → 2m×2 or 1m×3 for a distributed layout). The diffuser gives the even workshop wash + hides the channel. SKU/price PENDING — Alvin sourcing from an existing supplier; est $25–45."),
