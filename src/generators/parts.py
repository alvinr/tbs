...
PARTS: list[Part] = [
    ...
    Part("bolt-m8x20", "M8×1.25 × 20 hex bolt, 304 SS (A2-70) — cross-slide joints J1/J2", "fasteners-hardware",
         "film", 32, "ea", 16 / 50, 16 / 50, "McMaster-Carr",
         spec="J1 (skate carriage → Z-slide) + J2 (X-way → Z-carriage) — 4× M8×20 each, 2 joints/corner ×4 = 32. Threads into the tapped 304 flat-bar slide; flat + split washer, 24 N·m, Loctite 243 (Sheet 8). 304 SS A2-70 (wet cyanotype zone; no chloride → 316 unneeded). SKU + price EST — source the M8×20 304 A2-70 (McMaster 18-8 metric hex, e.g. 91290A-series) at purchase.", note="SKU pending — confirm the M8×20 304 hex SKU + price at purchase (est $16/pack of 50)."),
    Part("washer-m8-flat", "M8 flat washer, 304 SS", "fasteners-hardware",
         "film", 32, "ea", 9 / 100, 9 / 100, "McMaster-Carr",
         spec="Flat washer under each J1/J2 M8×20 head (Sheet 8). 304 SS, wet zone. SKU + price EST — confirm at purchase.", note="SKU pending — M8 304 flat washer (est $9/pack of 100)."),
    Part("washer-m8-split", "M8 split lock washer, 304 SS", "fasteners-hardware",
         "film", 32, "ea", 11 / 100, 11 / 100, "McMaster-Carr",
         spec="Split lock washer under each J1/J2 M8×20 (with Loctite 243 — belt + suspenders vs transport vibration; Sheet 8). 304 SS, wet zone. SKU + price EST — confirm at purchase.", note="SKU pending — M8 304 split lock washer (est $11/pack of 100)."),
    ...
    Part("nut-m4-plain", "M4×0.7 hex nut, 304 SS", "fasteners-hardware",
         "film", 24, "ea", 4.5 / 100, 4.5 / 100, "McMaster-Carr",
         spec="Plain hex nut under the cam-clamp mount tab — M4×12 bolt-through (2/clamp ×12 = 24). 304 SS, wet zone. Pitch M4×0.7. SKU + price EST — confirm the M4 304 nut at purchase.", note="SKU pending — M4×0.7 304 hex nut (est $4.50/pack of 100)."),
