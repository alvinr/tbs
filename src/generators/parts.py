...
PARTS: list[Part] = [
    ...
    Part("shelf-wall-eye-bolt", "M8 SS eye bolt — chain wall anchor", "fasteners-hardware",
         "shelf", 2, "ea", 7.80, 7.80, "McMaster-Carr", part_no="4843T13", url="https://www.mcmaster.com/4843T13/", spec="304 SS M8 eye bolt into the backing-plate M8 weld-nut ~230mm above the hinge (reuses the M8 wall standard) — the chain's top anchor. $7.80 ea."),
    ...
    Part("shelf-hinge-screws", "1/4\"-20 SS machine screws — hinge → tee-nuts", "fasteners-hardware",
         "shelf", 4, "ea", 0.40, 0.60, "McMaster-Carr", spec="1/4-20 × ~3/4in SS machine screw — hinge leaf into the ply tee-nut row (4 bolts). SS is the splash-zone exception to the zinc ply-mount screw (panel-machine-screws). ~4 off. SKU pending — source a 1/4-20 × 3/4 SS machine screw."),
    ...
    Part("shelf-wall-backing", "Shelf mount backing plates, 8mm steel (welded, ×3)", "steel-structural",
         "shelf", 3, "ea", 6, 10, "Local fab", spec="Flat 8mm steel backing plates welded to the pinhole-wall interior crests — one behind the hinge cleat + one per chain wall-anchor (M8 eye bolt) — flat, solid load anchors with M8 weld-nuts."),
    Part("bolt-m8-wall", "M8×1.25 × 25 hex bolt, Grade 8.8 zinc — shelf hinge cleat", "fasteners-hardware",
         "shelf", 6, "ea", 18.51 / 50, 18.51 / 50, "McMaster-Carr", part_no="91280A534", url="https://www.mcmaster.com/91280A534/", spec="Clamps the shelf hinge cleat (6mm) to its welded 8mm backing plate (M8 weld-nut). Grip ≈ 14mm → M8×25. Pitch M8×1.25 coarse. $18.51/pack of 50 (same 91280A534 as the film ICP-14 fixing)."),
    Part("nut-m8-plain", "M8×1.25 hex nut, plain SS", "fasteners-hardware",
         "shelf", 6, "ea", 7.53 / 100, 7.53 / 100, "McMaster-Carr", part_no="90591A161", url="https://www.mcmaster.com/90591A161/", spec="Plain hex nut — shelf hinge-cleat bolts. Pitch M8×1.25 coarse — confirmed vs 90591A161 PDF 2026-07-29 (matches the bolt). $7.53/pack of 100."),
    Part("washer-m8-flat-shelf", "M8 flat washer, SS", "fasteners-hardware",
         "shelf", 6, "ea", 3.32 / 100, 3.32 / 100, "McMaster-Carr", part_no="91166A270", url="https://www.mcmaster.com/91166A270/", spec="Flat washer (1/bolt) — shelf hinge-cleat bolts (dry). Distinct SKU from the film-plane wet-zone washer-m8-flat (98689A673): separate keys so the by-type BOM doesn't blend the two M8 flat washers. $3.32/pack of 100."),
