...
PARTS: list[Part] = [
    ...
    Part("corridor-panel-ply-18", "Corridor plumbing-panel ply (23/32\" exterior)", "timber-ply",
         ...
         spec='4×8 ft 23/32\" (18mm) RTD Southern Yellow Pine exterior sheathing — rear backing board + drain-riser spine + spacer offcuts. STANDARD exterior per project rule. Firm $29.30 (Home Depot 2026-07-23). Seal cut edges.',
         panel="Corridor"),
    ...
    Part("alu-angle-2x2", 'Aluminum angle 2"×2"×1/8" (6061-T6, plain) — 16 ft lengths', "aluminum",
         ...
         spec="6061-T6 angle (NOT 2024/7075 — corrosion + weldability). PLAIN mill finish (NOT anodized) — the film-plane PERIMETER FRAME, EXPENDABLE (inspect-annually / replace-on-pitting; bare 6061 pits sooner than anodized in the splash zone, so a shorter interval — anodizing is an option for longer life). 1/8in wall — frame sag is optically irrelevant at f/1088 and the ACM backing carries flatness, so only the wall is thinned; the 2in leg (the capture channel) is kept. WELD-FREE cut plan from 3× 16 ft (192\") lengths: 2 lengths → the two horizontal edges (4,499mm each, one per length, 378mm offcut); 1 length → both vertical edges (2,094mm ×2 from one 16 ft). No mid-span splices — only the 4 corner joints are welded/bolted. Metal Supermarkets 192\" @ $176.06 (2026-07-31). One frame — re-order to replace.",
    ...
    Part("wall-seat-saddle", "Mild steel plate 8mm (laser/plasma cut + welded)", "steel-structural",
         ...
         spec="ICP-11: per saddle = back-plate 150×200 + exterior plate 150×200 + gusset triangle 200×110 (all 8mm A36) + seat 150×110 (10mm A36); A36 mild steel (Alvin 2026-07-21); ~21 kg over 6 saddles. CUT-FROM: 8mm sheet 610×760mm (24×30in) nests all 12 back/exterior plates + 6 gussets (from 3× 200×110); 10mm sheet 610×254mm (24×10in) nests the 6 seats. Plus laser/plasma cut + weld — get a fab quote (the $53/saddle est covers material + fab)."),
    Part("bolt-m12x65", "M12×65 hex through-bolt, Grade 8.8 zinc, partial-thread", "fasteners-hardware",
